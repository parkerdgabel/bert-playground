...
class FusedMultiHeadAttention(nn.Module):
    ...
    def __call__(
        ...
        context = context.transpose(0, 2, 1, 3).reshape(batch_size, seq_length, hidden_size)
...
class ModernBertModel(nn.Module):
    ...
    def __call__(
        ...
        for layer in self.encoder.layers:
            hidden_states, attn_weights = layer(hidden_states, extended_attention_mask, training)
    ...
    def from_pretrained(cls, model_path: Union[str, Path], **kwargs):
        ...
        if model_path.is_dir():
            ...
            model = cls(config)
            logger.warning("HuggingFace weight conversion not implemented, using random initialization")
...
def create_model(config: Optional[ModernBertConfig] = None, **kwargs) -> ModernBertModel:
