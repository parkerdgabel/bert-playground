...
from ..heads.head_registry import CompetitionType, get_head_registry
...
class BertWithHead(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized BertWithHead: {bert.__class__.__name__} + {head.__class__.__name__}"
        )
    ...
    def __call__(
        ...
        **kwargs,
    ) -> dict[str, mx.array] | tuple[mx.array, ...]:
    ...
    def compute_loss(
        self, predictions: dict[str, mx.array], labels: mx.array, **kwargs
    ) -> mx.array:
    ...
    @classmethod
    def from_pretrained(cls, model_path: str | Path, **kwargs) -> "BertWithHead":
        ...
        if "pooling_type" in head_config_dict:
            head_config_dict["pooling_type"] = PoolingType(
                head_config_dict["pooling_type"]
            )
        if "activation" in head_config_dict:
            from ..heads.base_head import ActivationType

            head_config_dict["activation"] = ActivationType(
                head_config_dict["activation"]
            )
...
def create_bert_with_head(
    ...
    head_type: HeadType | str | None = None,
