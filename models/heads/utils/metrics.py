...
def _to_numpy(array: mx.array | np.ndarray) -> np.ndarray:
...
def f1_score(
    predictions: mx.array | np.ndarray,
    targets: mx.array | np.ndarray,
    average: str = "binary",
) -> float:
    """Compute F1 score.
...
def mape(
    ...
    return float(
        np.mean(np.abs((targets_np[mask] - pred_np[mask]) / targets_np[mask])) * 100
    )
...
def mean_average_precision(
    scores: mx.array | np.ndarray, targets: mx.array | np.ndarray
) -> float:
...
def compute_metrics(
    predictions: dict[str, mx.array | np.ndarray],
    targets: mx.array | np.ndarray,
    metrics: list[str],
    **kwargs,
) -> dict[str, float]:
    ...
    for metric_name in metrics:
        try:
            if metric_name == "accuracy":
                ...
                if pred is not None:
                    average = (
                        metric_name.split("_")[1] if "_" in metric_name else "binary"
                    )
