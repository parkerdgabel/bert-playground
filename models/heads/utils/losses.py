...
class FocalLoss(nn.Module):
    ...
    def __init__(
        self,
        alpha: Optional[Union[float, mx.array]] = None,
        gamma: float = 2.0,
        reduction: str = "mean",
        num_classes: Optional[int] = None
    ):
    ...
    def _multiclass_focal_loss(self, predictions: mx.array, targets: mx.array) -> mx.array:
        ...
        # Apply alpha weighting if specified
        if self.alpha is not None:
            if isinstance(self.alpha, mx.array) and self.alpha.shape[0] == self.num_classes:
                # Per-class alpha
...
class LabelSmoothingLoss(nn.Module):
    ...
    def __call__(self, predictions: mx.array, targets: mx.array) -> mx.array:
        ...
        smoothed_targets = (1.0 - self.smoothing) * targets_one_hot + \
