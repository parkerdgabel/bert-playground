...
from .config import HeadConfig, RegressionConfig
...
class RegressionHead(BaseHead):
    ...
    def __init__(self, config: RegressionConfig | HeadConfig):
    ...
    def _huber_loss(
        self, predictions: mx.array, targets: mx.array, delta: float
    ) -> mx.array:
        """Compute Huber loss.
...
class OrdinalRegressionHead(BaseHead):
    ...
    def __init__(self, config: HeadConfig | RegressionConfig, num_classes: int):
    ...
    def compute_loss(
        ...
        loss = -(
            cumulative_targets * mx.log(probs)
...
class QuantileRegressionHead(BaseHead):
    ...
    def __init__(
        self,
        config: HeadConfig | RegressionConfig,
    ...
    def _build_output_layer(self):
        ...
        self.quantile_heads = nn.Linear(
            self.projection_output_size, len(self.quantiles), bias=self.config.use_bias
        )
    ...
    def compute_loss(
        ...
        # Pinball loss
        loss = mx.maximum(
            quantiles_expanded * errors, (quantiles_expanded - 1) * errors
        )
...
def create_regression_head(
    config: RegressionConfig | HeadConfig, head_type: str | None = None
) -> BaseHead:
...
def create_standard_regression_head(input_size: int, **kwargs) -> RegressionHead:
...
def create_quantile_regression_head(
    input_size: int, quantiles: list | None = None, **kwargs
