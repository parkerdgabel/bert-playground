...
class HeadConfig:
    ...
    def __post_init__(self):
        ...
        if self.head_type not in valid_head_types:
            raise ValueError(
                f"Invalid head_type: {self.head_type}. Must be one of {valid_head_types}"
            )
        ...
        if self.pooling_type not in valid_pooling_types:
            raise ValueError(
                f"Invalid pooling_type: {self.pooling_type}. Must be one of {valid_pooling_types}"
            )
        ...
        if self.activation not in valid_activations:
            raise ValueError(
                f"Invalid activation: {self.activation}. Must be one of {valid_activations}"
            )
...
def get_base_config(input_size: int, output_size: int, **kwargs) -> HeadConfig:
    ...
    Returns:
        HeadConfig instance
    """
    return HeadConfig(input_size=input_size, output_size=output_size, **kwargs)
...
def get_regression_config(input_size: int, **kwargs) -> RegressionConfig:
    ...
    """
    return RegressionConfig(input_size=input_size, output_size=1, **kwargs)
...
def get_multiclass_classification_config(
...
def get_multilabel_classification_config(
    input_size: int, num_labels: int
) -> ClassificationConfig:
