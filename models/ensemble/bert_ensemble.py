"""
BERT ensemble model implementation.

This module provides the core ensemble model functionality for combining
multiple BERT models' predictions.
"""
...
class BERTEnsembleModel:
    ...
    def add_model(
        self, model: Model, weight: float = 1.0, config: dict[str, Any] | None = None
    ):
        """
            ...
            for seed in self.config.random_seeds:
                for max_len in self.config.max_seq_lengths:
                    for dropout in self.config.dropout_rates:
                        ...
                        logger.info(
                            f"Created {model_type} model: seed={seed}, "
    ...
    def create_checkpoint_average_model(
        self, checkpoint_dir: Path, last_n: int | None = None, best_n: int | None = None
    ) -> Model:
        """
    ...
    def predict(
        self, dataloader: DataLoader, return_all: bool = False
