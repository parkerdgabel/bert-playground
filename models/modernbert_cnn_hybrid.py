    ...
    def __call__(self, x: mx.array) -> List[mx.array]:
        ...
        for kernel_size, conv_linear in self.conv_layers:
            ...
            for i in range(seq_len):
                ...
                window = x_padded[
                    :, i : i + kernel_size, :
                ]  # (batch, kernel_size, hidden_size)
                # Flatten window
                window_flat = window.reshape(
                    batch_size, -1
                )  # (batch, kernel_size * hidden_size)
            ...
            # Stack outputs
            conv_output = mx.stack(
                conv_outputs, axis=1
            )  # (batch, seq_len, num_filters)
...
class DilatedConvBlock(nn.Module):
    ...
    def __init__(
        ...
        for d in dilation_rates:
            # Effective kernel size with dilation: (kernel_size - 1) * d + 1
        ...
        for dilation, conv_linear in self.dilated_convs:
            ...
            for i in range(seq_len):
                ...
                if len(indices) == self.kernel_size:
                    ...
                    conv_outputs.append(
                        mx.zeros((batch_size, conv_linear.weight.shape[0]))
                    )
...
class AttentionFusion(nn.Module):
    ...
    def __call__(self, features: List[mx.array]) -> mx.array:
        ...
        # Note: mx.stack(features) would be used here if needed
...
class CNNEnhancedModernBERT(nn.Module):
    ...
    def __init__(self, config: CNNHybridConfig):
        ...
        self.feature_projection = nn.Sequential(
            nn.Linear(
                config.hidden_size + fusion_output_dim, config.fusion_hidden_size
            ),
    ...
    def extract_cnn_features(
        ...
        if attention_mask is not None:
            ...
            all_conv_outputs = [output * mask_expanded for output in all_conv_outputs]
    ...
    @classmethod
    def from_pretrained(
        cls, path_or_name: str, num_labels: int = 2, **kwargs
        ...
        if path.exists() and path.is_dir():
            ...
            model = cls(config)
            logger.info(
                f"Initialized CNN-Enhanced ModernBERT with config from {path_or_name}"
            )
