...
from .layers import LoRALinear, QLoRALinear, MultiLoRALinear
...
class LoRAAdapter:
    ...
    def __init__(self, model: nn.Module, config: Union[LoRAConfig, QLoRAConfig]):
        ...
        for name, module in self.model.named_modules():
            if self._should_inject_lora(name, module):
                ...
                if new_module is not None:
                    ...
                    if verbose:
                        print(f"Injected LoRA into {name}: +{params_added:,} parameters")
    ...
    def _should_inject_lora(self, name: str, module: nn.Module) -> bool:
        ...
        if isinstance(module, (LoRALinear, QLoRALinear, MultiLoRALinear)):
            return False
    ...
    def _create_lora_module(
        ...
        name: str, 
        module: nn.Module
    ) -> Optional[Union[LoRALinear, QLoRALinear]]:
    ...
    def save_all_adapters(self, directory: str) -> None:
        ...
        for name, adapter in self.adapters.items():
            ...
            safetensors.mlx.save_file(
                state_dict, 
                os.path.join(directory, f"{name}_adapter.safetensors")
            )
