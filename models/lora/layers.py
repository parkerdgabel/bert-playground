...
class LoRALinear(nn.Module):
    ...
    def _init_lora_layers(self):
        ...
        if init_method == "gaussian":
            ...
            self.lora_A = mx.random.normal(shape=(self.in_features, self.r), scale=std)
        elif init_method == "uniform":
            bound = 1 / math.sqrt(self.in_features)
            self.lora_A = mx.random.uniform(
                low=-bound, high=bound, shape=(self.in_features, self.r)
            )
        else:  # bert-style
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        # LoRA path
        x_dropped = self.lora_dropout(x) if self.lora_dropout is not None else x
        ...
        if self.config.use_dora:
            ...
            weight_norm = mx.sqrt(
                mx.sum(self.base_layer.weight**2, axis=1, keepdims=True)
            )
...
class QLoRALinear(nn.Module):
    ...
    def __init__(
        ...
        out_features: int,
        config: QLoRAConfig,
        base_layer: nn.Linear | QuantizedLinear | None = None,
        device: mx.Device | None = None,
    ):
        super().__init__()
    ...
    def _init_lora_layers(self):
        ...
        self.lora_A = mx.random.normal(
            shape=(self.in_features, self.r), scale=std, dtype=self.compute_dtype
        )

        # LoRA B matrix (initialized to zero)
        self.lora_B = mx.zeros((self.r, self.out_features), dtype=self.compute_dtype)

        # Optional bias
        if self.config.lora_bias_trainable:
            self.lora_bias = mx.zeros((self.out_features,), dtype=self.compute_dtype)
        else:
            self.lora_bias = None
...
class MultiLoRALinear(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.combination_type == "average":
            ...
            weights = [
                self.adapter_weights.get(name, 1.0) for name in self.active_adapters
            ]
            ...
            if not hasattr(self, "projection"):
                self.projection = nn.Linear(
                    len(self.active_adapters) * self.out_features, self.out_features
                )
