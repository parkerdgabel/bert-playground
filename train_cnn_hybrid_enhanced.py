...
from utils.loss_functions import get_titanic_loss, get_loss_function
from utils.gradient_utils import create_gradient_monitor, GradientMonitor
...
class EnhancedHybridModelTrainer:
    ...
    def __init__(
        ...
        **model_kwargs
        ...
        logger.info(f"Enhanced trainer initialized for {model_type} model with {loss_function} loss")
    ...
    def train(self):
        ...
        config = {
            ...
            'max_gradient_norm': self.gradient_monitor.max_norm if self.gradient_monitor else None,
        ...
        logger.info(f"Starting enhanced training for {self.epochs} epochs ({total_steps} steps)")
            ...
            with tqdm(
                ...
                for batch_idx, batch in enumerate(pbar):
                    ...
                    if self.mlflow_tracker and self.global_step % 10 == 0:
                        ...
                        self.mlflow_tracker.log_metrics(train_metrics, step=self.global_step)
                    ...
                    if self.global_step % 100 == 0:
                        ...
                        if self.mlflow_tracker:
                            ...
                            mlflow_metrics = {k: v for k, v in metrics.items() if not isinstance(v, list)}
                            self.mlflow_tracker.log_metrics(mlflow_metrics, step=self.global_step)
        ...
        if self.mlflow_tracker:
            # Filter out list values that MLflow can't handle
            mlflow_final_metrics = {k: v for k, v in final_metrics.items() if not isinstance(v, list)}
        ...
        if self.gradient_monitor:
            gradient_summary = self.gradient_monitor.get_summary_stats()
            logger.info(f"Gradient clipping summary: {gradient_summary['clip_percentage']:.1f}% of steps clipped")
            logger.info(f"Gradient norm trend: {gradient_summary['recent_gradient_trend']}")
        
        return final_metrics


def main():
    parser = argparse.ArgumentParser(description="Enhanced CNN-BERT training with advanced features")
    ...
    parser.add_argument('--loss-function', 
                        choices=['focal', 'weighted_ce', 'label_smooth', 'focal_smooth', 'dice', 'tversky', 'adaptive', 'standard'],
                        default='focal',
                        help='Loss function to use')
    
    # Gradient clipping arguments
    parser.add_argument('--gradient-clipping', action='store_true', default=True,
    ...
    # Output arguments
    parser.add_argument('--output-dir', default='./output/cnn_hybrid_enhanced',
                        help='Output directory')
    parser.add_argument('--experiment-name', default='titanic_cnn_hybrid_enhanced',
                        help='MLflow experiment name')
    
    # MLflow arguments
    parser.add_argument('--enable-mlflow', action='store_true', default=True,
                        help='Enable MLflow tracking')
    parser.add_argument('--mlflow-base-dir', default='./mlflow',
                        help='Base directory for MLflow')
    ...
    if trainer.mlflow_tracker:
        logger.info(f"  - MLflow tracking: {trainer.mlflow_tracker.config.tracking_uri}")
