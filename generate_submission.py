...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Kaggle submission from trained model"
    )
    parser.add_argument("model_path", type=str, help="Path to model checkpoint")
    parser.add_argument(
        "--test-path",
        type=str,
        default="data/titanic/test.csv",
        help="Path to test data CSV",
    )
    parser.add_argument("--output", type=str, help="Output submission filename")
    parser.add_argument(
        "--submit", action="store_true", help="Submit to Kaggle after generation"
    )
    ...
    try:
        ...
        submission_path = generate_submission_only(
            args.model_path, args.test_path, args.output
        )
