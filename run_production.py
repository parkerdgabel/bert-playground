...
def main():
    parser = argparse.ArgumentParser(
        description="Production training runner for Titanic ModernBERT"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--experiment-name", default="titanic_modernbert", help="MLflow experiment name"
    )
    parser.add_argument(
        "--enable-mlflow", action="store_true", help="Enable MLflow tracking"
    )
    parser.add_argument(
        "--predict", action="store_true", help="Run prediction after training"
    )
    ...
    print("Dataset: Titanic (891 training samples, ~2673 with augmentation)")
    print("Configuration:")
    ...
    print(
        f"Estimated time: {total_steps * 0.5:.1f} - {total_steps * 1.5:.1f} seconds\n"
    )
