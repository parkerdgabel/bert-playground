...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all models and submit best to Kaggle")
    parser.add_argument("--submit-best", action="store_true", help="Submit best model to Kaggle")
    ...
    try:
        ...
        for _, row in comparison_df.iterrows():
            table.add_row(
                row['model_path'].split('/')[-2] if '/' in row['model_path'] else row['model_path'],
                row['model_type'],
                f"{row['accuracy']:.4f}",
                f"{row['f1']:.4f}",
                f"{row.get('auc', 0):.4f}"
