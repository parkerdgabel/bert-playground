...
def run_command(
    ...
    return runner.invoke(
        app, args, input=input, env=env, catch_exceptions=catch_exceptions
    )


def assert_success(result: Any, expected_output: str | None = None):
...
def assert_failure(result: Any, expected_error: str | None = None, exit_code: int = 1):
    ...
    if expected_error:
        ...
        assert expected_error in error_output, (
            f"Expected '{expected_error}' in error output, got: {error_output}"
        )
...
def parse_progress_output(output: str) -> list[dict[str, Any]]:
    ...
    for match in re.finditer(pattern, output):
        progress_updates.append(
            {"percentage": int(match.group(1)), "bar": match.group(2)}
        )
...
def mock_console_output():
    ...
    def capture_print(*args, **kwargs):
        captured_output.append({"args": args, "kwargs": kwargs})
...
def assert_file_created(path: str | Path, content: str | None = None):
...
def assert_files_created(paths: list[str | Path]):
    """Assert multiple files were created."""
    for path in paths:
        assert_file_created(path)


def assert_config_valid(config_path: str | Path, expected_keys: list[str]):
...
def compare_configs(
    config1: dict, config2: dict, ignore_keys: list[str] = None
) -> bool:
...
def assert_performance(
    execution_time: float, max_time: float, operation: str = "Command"
):
