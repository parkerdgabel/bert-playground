...
class TestConverterPerformance:
    """Performance tests for text converters."""
    
    def test_large_dataset_conversion(self):
        """Test conversion performance on large dataset."""
        ...
        assert elapsed < 5.0  # Should convert 10k rows in less than 5 seconds
        
        # Throughput
        throughput = len(df) / elapsed
        assert throughput > 2000  # >2000 rows per second
        
    def test_memory_efficiency(self):
        """Test memory efficiency of conversion."""
        import psutil
        import os
        ...
        df = create_sample_dataframe(size=5000)
        converter = TabularTextConverter()
