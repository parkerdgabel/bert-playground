...
class TestMemoryPerformance:
    ...
    def test_large_allocation_performance(self, measure_memory_allocation_time):
        ...
        # Time large allocations
        time_taken = measure_memory_allocation_time(
            manager,
            num_allocations=10,
            shape=(1000, 1000),
            dtype=mx.float32
        ...
        assert time_taken < 1.0  # Less than 1 second for 10 large allocations
        
    def test_cache_performance_under_load(self, check_memory_usage):
        ...
        for i in range(1000):
            key = f"tensor_{i % 100}"  # Reuse some keys
            
            # Time cache operations
            import time
            start = time.time()
            
            tensor = manager.get_cached_tensor(key)
            if tensor is None:
                tensor = manager.allocate_tensor(shape=(100, 100), dtype=mx.float32)
                manager.cache_tensor(key, tensor)
                
            elapsed = time.time() - start
            cache_times.append(elapsed)
            
        # Average cache operation should be fast
        avg_time = sum(cache_times) / len(cache_times)
        assert avg_time < 0.001  # Less than 1ms average
        
        # Cache should have good hit rate
        stats = manager.get_cache_statistics()
        assert stats['cache_hit_rate'] > 0.5  # >50% hit rate with key reuse