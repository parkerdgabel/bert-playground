...
class TestStreamingPerformance:
    ...
    def test_high_throughput_streaming(self):
        """Test streaming at high throughput."""
        spec = create_dataset_spec(num_samples=10000)
        dataset = StreamingDataset(spec, size=10000, fast_mode=True)
        ...
        try:
            ...
            assert throughput > 5000  # >5000 samples/sec
    ...
    def test_memory_bounded_streaming(self):
        ...
        dataset = StreamingDataset(spec, size=100000)
        ...
        try:
            # Monitor memory while streaming
            max_memory = 0
            ...
            assert max_memory < 150  # Less than 150MB
            
        finally:
            pipeline.stop()
            
    def test_adaptive_performance(self):
        """Test adaptive performance under varying load."""
        spec = create_dataset_spec(num_samples=1000)
        dataset = StreamingDataset(spec, size=1000)
        ...
        try:
            ...
            for phase in range(3):
                ...
                elapsed = time.time() - start_time
                throughput = len(phase_samples) / elapsed
                throughputs.append(throughput)
                
            # Pipeline should adapt to consumption rate
            assert throughputs[0] > throughputs[1] > throughputs[2]
            
            # Check adaptation happened
            adaptation_stats = pipeline.get_adaptation_statistics()
