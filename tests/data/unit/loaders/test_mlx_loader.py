...
@pytest.mark.slow
class TestMLXLoaderPerformance:
    """Performance tests for MLX data loader."""
    
    def test_large_batch_loading(self, measure_throughput):
        ...
        # Measure throughput
        throughput = measure_throughput(loader, num_batches=10)
        
        # Should achieve reasonable throughput
        assert throughput > 1000  # samples per second
        
    def test_memory_efficiency_large_dataset(self, check_memory_usage):
        """Test memory efficiency with large dataset."""
        spec = create_dataset_spec(num_samples=50000)
        dataset = MockKaggleDataset(spec, size=50000)
        ...
        memory_before = loader.get_memory_info()
        
        for i, batch in enumerate(loader):
            if i >= 10:
                break
                
        memory_after = loader.get_memory_info()
        
        # Memory should not grow excessively
        memory_growth = memory_after['allocated_mb'] - memory_before['allocated_mb']
        assert memory_growth < 200  # Less than 200MB growth
        
    def test_prefetching_performance(self):
        """Test performance improvement with prefetching."""
        spec = create_dataset_spec(num_samples=1000)
        dataset = MockKaggleDataset(spec, size=1000)
        
        # Without prefetching
        config_no_prefetch = create_mlx_loader_config(
            batch_size=32,
            prefetch_size=0,
        )
        loader_no_prefetch = MLXDataLoader(dataset, config_no_prefetch)
        
        start = time.time()
        list(loader_no_prefetch)
        time_no_prefetch = time.time() - start
        
        # With prefetching
        config_prefetch = create_mlx_loader_config(
            batch_size=32,
            prefetch_size=4,
        )
        loader_prefetch = MLXDataLoader(dataset, config_prefetch)
        
        start = time.time()
        list(loader_prefetch)
        time_prefetch = time.time() - start
        
        # Prefetching should be faster (or at least not slower)
        assert time_prefetch <= time_no_prefetch * 1.1  # Allow 10% variance