...
from data.core.base import CompetitionType, DatasetSpec, KaggleDataset
...
class MockKaggleDataset(KaggleDataset):
    ...
    def _generate_data(self):
        ...
        data = {f"feature_{i}": np.random.randn(self.size) for i in range(num_features)}
...
class SyntheticTextDataset:
    ...
    def __getitem__(self, idx: int) -> dict[str, mx.array]:
        ...
        seq_len = mx.random.randint(
            self.sequence_length // 2, self.sequence_length
        ).item()
...
class FaultyDataset:
    ...
    def __init__(self, spec: DatasetSpec, error_rate: float = 0.1, **kwargs):
    ...
    def __getitem__(self, idx: int) -> dict[str, mx.array] | None:
...
def create_kaggle_like_dataset(
    ...
    **kwargs,
    ...
    competition_type = competition_type_map.get(
        task_type, CompetitionType.BINARY_CLASSIFICATION
    )
    ...
    spec = DatasetSpec(
        ...
        numerical_columns=[
            f"num_{i}" for i in range(num_features - 2)
        ],  # Minus categorical columns
    ...
    kwargs_filtered = {
        k: v for k, v in kwargs.items() if k not in ["size", "cache_dir"]
    }
...
def create_classification_dataset(
    ...
    **kwargs,
    ...
    if dataset_type == "tabular":
        ...
        return ImbalancedDataset(num_samples=num_samples, **kwargs)
    else:
        raise ValueError(f"Unknown dataset type: {dataset_type}")


def create_regression_dataset(
    num_samples: int = 1000, dataset_type: str = "tabular", **kwargs
) -> Any:
    """Create regression dataset of specified type."""
    if dataset_type == "tabular":
        return SyntheticTabularDataset(
            num_samples=num_samples, task_type="regression", **kwargs
        )
    elif dataset_type == "text":
        return SyntheticTextDataset(
            num_samples=num_samples, task_type="regression", **kwargs
