...
def house_prices_dataset_spec():
    ...
    return DatasetSpec(
        ...
        text_columns=[],
        categorical_columns=["MSZoning", "Street", "LotShape", "Neighborhood", "KitchenQual"],
        numerical_columns=["LotArea", "OverallQual", "OverallCond", "YearBuilt", "TotalBsmtSF", "GrLivArea", "BedroomAbvGr"],
        num_classes=1,
    )
    ...
    class MockTokenizer:
        ...
        def __call__(self, texts, padding=True, truncation=True, max_length=512, return_tensors=None):
            ...
            for text in texts:
                ...
                input_ids = [0] + [self.vocab.get(token, 3) for token in tokens] + [1]  # CLS + tokens + SEP
                
                if padding and len(input_ids) < max_length:
                    attention_mask = [1] * len(input_ids) + [0] * (max_length - len(input_ids))
                    input_ids = input_ids + [2] * (max_length - len(input_ids))  # Pad with PAD token
                else:
...
def create_mock_dataset_files(base_dir: Path, competition_name: str, data: pd.DataFrame, target_col: str = None):
