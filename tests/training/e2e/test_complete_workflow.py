...
class TestCompleteWorkflow:
    ...
    def test_kaggle_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            trainer = create_trainer(model=model, config=config, trainer_type="kaggle")
    ...
    def test_resume_training(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Initial training
            trainer = create_trainer(model=model, config=config, trainer_type="base")
            ...
            trainer2 = create_trainer(
                model=new_model, config=config, trainer_type="base"
            )
        ...
        for trainer_type in trainer_types:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                trainer = create_trainer(
                    model=model, config=config, trainer_type=trainer_type
                )
