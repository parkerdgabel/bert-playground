...
class TestCompiledGradientClipping:
    ...
    def test_clip_gradients_with_tree_flatten(self):
        ...
        for key in grads:
            ...
            for param_key in grads[key]:
                assert clipped_grads[key][param_key].shape == grads[key][param_key].shape
    ...
    def test_gradient_norm_clipping(self):
        ...
        assert abs(float(clipped_norm) - max_norm) < 1e-4  # Clipped to max_norm (allow for float32 precision)
