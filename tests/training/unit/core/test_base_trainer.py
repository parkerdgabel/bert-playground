...
class TestBaseTrainer:
    ...
    def test_training_loop(self, tmp_path):
        ...
        )  # 0-based indexing, so after 2 epochs (0, 1) we're at epoch 1
    ...
    def test_checkpoint_saving(self, tmp_path):
        ...
        trainer.save_checkpoint(
            None
        )  # Path parameter is ignored in current implementation

        # The checkpoint will be saved as checkpoints/checkpoint-100 based on global_step
        checkpoint_path = (
            trainer.config.environment.output_dir / "checkpoints" / "checkpoint-100"
        )
    ...
    def test_checkpoint_loading(self, tmp_path):
        ...
        checkpoint_path = (
            trainer1.config.environment.output_dir
            / "checkpoints"
            / f"checkpoint-{trainer1.state.global_step}"
        )
    ...
    def test_resume_training(self, tmp_path):
        ...
        assert (
            trainer2.state.epoch == 3
        )  # 0-based indexing, so after 4 epochs we're at epoch 3
        assert trainer2.state.global_step > trainer1.state.global_step
