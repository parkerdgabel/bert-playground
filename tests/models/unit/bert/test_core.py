...
from models.bert import BertCore, ModernBertCore
...
class TestBertCore:
    ...
    def test_forward_pass(self, create_test_batch):
        ...
        batch = create_test_batch(
            batch_size=4, seq_length=32, vocab_size=config.vocab_size
        )
    ...
    def test_forward_without_attention_mask(self, create_test_batch):
        ...
        batch = create_test_batch(
            batch_size=2, seq_length=16, vocab_size=config.vocab_size
        )
...
class TestModernBertCore:
    ...
    def test_forward_pass(self, create_test_batch):
        ...
        batch = create_test_batch(
            batch_size=4, seq_length=64, vocab_size=config.vocab_size
        )
    ...
    def test_sliding_window_attention(self, sliding_window):
        ...
        config = create_modernbert_config(
            sliding_window=sliding_window, num_hidden_layers=2
        )
        bert = ModernBertCore(config)
...
class TestBertCoreComparison:
    ...
    def test_bert_vs_modernbert_outputs(self, create_test_batch):
        ...
        # Same input
        batch = create_test_batch(batch_size=2, seq_length=32, vocab_size=1000)
        ...
        assert (
            ...
            == modernbert_outputs.last_hidden_state.shape
        )
...
class TestBertCoreMemory:
    ...
    def test_memory_usage(self, memory_profiler):
        ...
        batch = create_test_batch(
            batch_size=8, seq_length=128, vocab_size=config.vocab_size
        )
