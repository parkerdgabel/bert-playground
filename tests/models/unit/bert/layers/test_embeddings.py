...
class TestBertEmbeddings:
    ...
    def test_initialization(self):
        ...
        assert embeddings.word_embeddings.weight.shape == (
            config.vocab_size,
            config.hidden_size,
        )
        assert embeddings.position_embeddings.weight.shape == (
            config.max_position_embeddings,
            config.hidden_size,
        )
        assert embeddings.token_type_embeddings.weight.shape == (
    ...
    def test_with_token_type_ids(self):
        ...
        batch_size, seq_length = 4, 32
        input_ids = mx.random.randint(0, config.vocab_size, (batch_size, seq_length))
        token_type_ids = mx.random.randint(
            0, config.type_vocab_size, (batch_size, seq_length)
        )
    ...
    def test_embedding_weight_sharing(self):
        ...
        assert mx.allclose(
            embeddings.position_embeddings.weight, position_weight_initial
        )
