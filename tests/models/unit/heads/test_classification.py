...
from tests.models.fixtures.data import create_classification_targets
...
class TestBinaryClassificationHead:
    ...
    def test_initialization(self):
        ...
        config = create_classification_config(
            hidden_size=768, num_labels=2, dropout_prob=0.1
        )
    ...
    def test_forward_pass(self, create_embeddings):
        ...
        config = create_classification_config(
            hidden_size=128, num_labels=2, pooling_type="cls"
        )
        head = BinaryClassificationHead(config)
        ...
        # Forward pass
        outputs = head(
            hidden_states=hidden_states, attention_mask=attention_mask, labels=labels
        )

        assert "logits" in outputs
        # Binary classification can output (batch_size,) or (batch_size, 1)
        assert outputs["logits"].shape == (batch_size,) or outputs["logits"].shape == (
            batch_size,
            1,
        )
    ...
    def test_pooling_strategies(self, create_embeddings):
        ...
        for pooling_type in ["cls", "mean", "max"]:
            config = create_classification_config(
                hidden_size=128, num_labels=2, pooling_type=pooling_type
            )
            head = BinaryClassificationHead(config)
    ...
    def test_with_masked_input(self, create_embeddings, create_attention_mask):
        ...
        config = create_classification_config(
            hidden_size=128, num_labels=2, pooling_type="mean"
        )
        head = BinaryClassificationHead(config)
    ...
    def test_gradient_flow(self, create_embeddings, check_gradients):
        ...
        def loss_fn(model, batch):
            outputs = model(
                hidden_states=batch["hidden_states"], labels=batch["labels"]
            )
            ...
            loss = -mx.mean(
                labels * mx.log(probs + 1e-8) + (1 - labels) * mx.log(1 - probs + 1e-8)
            )
            return loss
    ...
    def test_dropout_rates(self, dropout_prob, create_embeddings):
        ...
        config = create_classification_config(
            hidden_size=128, num_labels=2, dropout_prob=dropout_prob
        )
        head = BinaryClassificationHead(config)
...
class TestMulticlassClassificationHead:
    ...
    def test_initialization(self):
        ...
        config = create_classification_config(
            hidden_size=768, num_labels=10, dropout_prob=0.1
        )
    ...
    def test_forward_pass(self, create_embeddings):
        ...
        num_classes = 5
        config = create_classification_config(hidden_size=128, num_labels=num_classes)
        head = MulticlassClassificationHead(config)
    ...
    def test_loss_computation(self, create_embeddings):
        ...
        labels = create_classification_targets(
            batch_size, num_classes=num_classes, target_type="balanced"
        )
...
class TestHeadFactory:
    ...
    def test_factory_with_config(self):
        ...
        config = create_classification_config(
            hidden_size=256, num_labels=5, dropout_prob=0.2
        )
...
class TestClassificationHeadIntegration:
    ...
    def test_with_bert_outputs(self, mock_bert_model, create_test_batch):
        ...
        batch = create_test_batch(
            batch_size=4, seq_length=32, vocab_size=bert.config.vocab_size
        )
        bert_outputs = bert(
