...
class BrokenModel(nn.Module):
    ...
    def __init__(self, error_type: str = "forward", config: BertConfig | None = None):
        super().__init__()
...
# Factory functions
def create_test_bert_model(config: BertConfig | None = None) -> SimpleBertModel:
...
def assert_model_output_shapes(
    ...
    if expected_output_shape is not None:
        if isinstance(output, dict):
            ...
            assert any(v.shape == expected_output_shape for v in output.values()), (
