...
class TitanicPredictor:
    ...
    def load_model(self) -> TitanicClassifier:
        # Load BERT model
        bert_model = ModernBertMLX.from_pretrained(
            str(self.checkpoint_path / "bert")
        )
...
def create_submission(
    ...
    if ensemble_checkpoints:
        print(f"Creating ensemble predictions from {len(ensemble_checkpoints)} models...")
        ...
        print(f"Creating predictions from single model...")
    ...
    print(f"Survived: {submission['Survived'].sum()} ({submission['Survived'].mean():.2%})")
    print(f"Not survived: {(1 - submission['Survived']).sum()} ({(1 - submission['Survived']).mean():.2%})")
