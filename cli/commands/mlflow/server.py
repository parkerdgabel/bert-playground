...
def server_command(
    ...
    backend_store: Optional[Path] = typer.Option(None, "--backend-store", "-b", help="Backend store URI"),
    artifact_store: Optional[Path] = typer.Option(None, "--artifact-store", "-a", help="Artifact store URI"),
    ...
    console.print("This will start an MLflow tracking server with the specified configuration.")
...
@handle_errors
def restart_command(
    port: int = typer.Option(5000, "--port", "-p", help="Port of the MLflow server to restart"),
    force: bool = typer.Option(False, "--force", "-f", help="Force restart without confirmation"),
    timeout: int = typer.Option(30, "--timeout", "-t", help="Timeout in seconds for graceful shutdown"),
):
    ...
    if not force:
        confirm = typer.confirm(f"Are you sure you want to restart the MLflow server on port {port}?")
        if not confirm:
    ...
    console.print(f"This would restart the MLflow server on port {port} with a {timeout}s timeout.")
