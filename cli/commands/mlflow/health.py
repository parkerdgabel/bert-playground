...
def health_command(
    ...
    try:
        ...
        with console.status("[yellow]Running health checks...[/yellow]"):
            results = (
                health_checker.run_full_check()
            )  # run_full_check doesn't accept parameters
            ...
            for issue in results.get("issues", []):
                if issue.get("fixable"):
                    ...
                    if fix_result["success"]:
                        ...
                        print_warning(
                            f"Could not fix: {issue['description']} - {fix_result.get('error', 'Unknown error')}"
                        )
...
def _display_health_results(console, results):
    ...
    info_table = create_table(
        "MLflow Health Check Results", ["Check", "Status", "Details"]
    )
    ...
    for check_name, result in results.items():
        friendly_name = check_name_map.get(
