...
def list_experiments_command(
    tracking_uri: str | None = typer.Option(
        None, "--tracking-uri", "-u", help="MLflow tracking URI"
    ),
    view: str = typer.Option(
        "active", "--view", "-v", help="View type: active, deleted, or all"
    ),
    sort_by: str = typer.Option(
        "creation_time",
        "--sort",
        "-s",
        help="Sort by: name, creation_time, or last_update",
    ),
    limit: int = typer.Option(
        20, "--limit", "-l", help="Maximum number of experiments to show"
    ),
    filter_name: str | None = typer.Option(
        None, "--filter", "-f", help="Filter experiments by name pattern"
    ),
):
    ...
    experiments_table = create_table(
        "Experiments", ["ID", "Name", "Lifecycle", "Runs", "Created", "Last Updated"]
    )
...
@handle_errors
def clean_command(
    experiment_ids: list[str] | None = typer.Option(
        None, "--experiment", "-e", help="Specific experiment IDs to clean"
    ),
    older_than: int | None = typer.Option(
        None, "--older-than", "-o", help="Clean runs older than N days"
    ),
    failed_only: bool = typer.Option(
        False, "--failed-only", help="Only clean failed runs"
    ),
    ...
    keep_best: int = typer.Option(
        5, "--keep-best", "-k", help="Number of best runs to keep per experiment"
    ),
