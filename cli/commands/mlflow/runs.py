...
def list_runs_command(
    experiment_id: str = typer.Argument(
        ..., help="Experiment ID or name to list runs for"
    ),
    status: str | None = typer.Option(
        None, "--status", "-s", help="Filter by status: RUNNING, FINISHED, FAILED"
    ),
    metric: str | None = typer.Option(
        None, "--metric", "-m", help="Metric to display and sort by"
    ),
    ...
    show_params: bool = typer.Option(
        False, "--params", "-p", help="Show run parameters"
    ),
    ...
    if output_format == "table":
        ...
        # Mock JSON output
        console.print(
            '{"runs": [{"id": "a1b2c3d4e5f6", "status": "FINISHED", "accuracy": 0.9234}]}'
        )
