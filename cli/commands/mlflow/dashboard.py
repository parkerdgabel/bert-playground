...
def dashboard_command(
    experiment_id: str | None = typer.Option(
        None, "--experiment", "-e", help="Specific experiment to monitor"
    ),
    ...
    metrics: str | None = typer.Option(
        None, "--metrics", "-m", help="Comma-separated list of metrics to display"
    ),
    max_runs: int = typer.Option(
        50, "--max-runs", help="Maximum number of runs to display"
    ),
    theme: str = typer.Option(
        "dark", "--theme", "-t", help="Dashboard theme: dark or light"
    ),
):
