...
def convert_command(
    ...
    source_format: Optional[str] = typer.Option(None, "--from", "-f",
                                              help="Source format (auto-detect if not specified)"),
    target_format: str = typer.Option(..., "--to", "-T",
                                    help="Target format: mlx, pytorch, tensorflow, onnx"),
    model_type: Optional[str] = typer.Option(None, "--model-type", "-m",
                                           help="Model type if conversion needed"),
    config_file: Optional[Path] = typer.Option(None, "--config", "-c",
                                             help="Custom config file for conversion"),
    optimize: bool = typer.Option(True, "--optimize/--no-optimize",
                                help="Apply optimizations during conversion"),
    quantize: bool = typer.Option(False, "--quantize", "-q",
                                help="Quantize model during conversion"),
    bits: int = typer.Option(8, "--bits", "-b",
    ...
    if not _is_conversion_supported(source_format, target_format):
        print_error(f"Conversion from {source_format} to {target_format} is not supported")
    ...
    try:
        ...
        if source_format == "mlx" and target_format == "pytorch":
            _convert_mlx_to_pytorch(source, target, config, optimize, quantize, bits, verbose)
        
        elif source_format == "pytorch" and target_format == "mlx":
            _convert_pytorch_to_mlx(source, target, config, model_type, optimize, quantize, bits, verbose)
            ...
            _convert_huggingface_to_mlx(source, target, model_type, config, quantize, bits, verbose)
        
        else:
            print_error(f"Conversion from {source_format} to {target_format} not implemented")
            raise typer.Exit(1)
        
        # Validate if requested
        if validate:
            if _validate_conversion(source, target, source_format, target_format, verbose):
...
def _detect_format(path: Path, verbose: bool) -> Optional[str]:
...
def _convert_mlx_to_pytorch(source: Path, target: Path, config: dict,
                           optimize: bool, quantize: bool, bits: int, verbose: bool):
...
def _convert_pytorch_to_mlx(source: Path, target: Path, config: dict,
                           model_type: Optional[str], optimize: bool,
                           quantize: bool, bits: int, verbose: bool):
...
def _convert_mlx_to_onnx(source: Path, target: Path, config: dict,
                        optimize: bool, verbose: bool):
...
def _convert_pytorch_to_onnx(source: Path, target: Path, config: dict,
                            optimize: bool, verbose: bool):
...
def _convert_huggingface_to_mlx(source: Path, target: Path, model_type: Optional[str],
                               config: dict, quantize: bool, bits: int, verbose: bool):
...
def _infer_config_from_state_dict(state_dict: dict, model_type: Optional[str]) -> dict:
...
def _convert_hf_config_to_mlx(hf_config: Any, model_type: Optional[str]) -> dict:
...
def _convert_hf_weights_to_mlx(state_dict: dict, model_type: Optional[str], 
                              verbose: bool) -> dict:
...
def _validate_conversion(source: Path, target: Path, source_format: str,
                        target_format: str, verbose: bool) -> bool:
    ...
    try:
        ...
        if target.is_dir():
            ...
            for file in required_files:
                if not any((target / f).exists() for f in [file, file.replace('.', '_')]):
                    if verbose:
