...
def serve_command(
    ...
    port: int = typer.Option(
        8000, "--port", "-p", help="Port to serve on", callback=validate_port
    ),
    ...
    batch_size: int = typer.Option(
        32, "--batch-size", "-b", help="Maximum batch size for inference"
    ),
    max_length: int = typer.Option(256, "--max-length", help="Maximum sequence length"),
    timeout: int = typer.Option(
        30, "--timeout", "-t", help="Request timeout in seconds"
    ),
    cors: bool = typer.Option(True, "--cors/--no-cors", help="Enable CORS headers"),
    auth_token: str | None = typer.Option(
        None, "--auth-token", help="Optional authentication token"
    ),
    ...
    ssl_cert: Path | None = typer.Option(
        None, "--ssl-cert", help="Path to SSL certificate"
    ),
    ssl_key: Path | None = typer.Option(
        None, "--ssl-key", help="Path to SSL private key"
    ),
    model_name: str | None = typer.Option(
        None, "--model-name", "-n", help="Model name for API responses"
    ),
    cache_size: int = typer.Option(
        100, "--cache-size", help="Number of predictions to cache"
    ),
    enable_metrics: bool = typer.Option(
        True, "--metrics/--no-metrics", help="Enable Prometheus metrics endpoint"
    ),
    health_check: bool = typer.Option(
        True, "--health/--no-health", help="Enable health check endpoint"
    ),
    docs: bool = typer.Option(
        True, "--docs/--no-docs", help="Enable API documentation"
    ),
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Show detailed server information"
    ),
):
    ...
    try:
        ...
        console.print("  • POST   /predict      - Single prediction")
        console.print("  • POST   /predict/batch - Batch predictions")
        if health_check:
            console.print("  • GET    /health       - Health check")
        if enable_metrics:
            console.print("  • GET    /metrics      - Prometheus metrics")
        if docs:
            console.print("  • GET    /docs         - Interactive API docs")
            console.print("  • GET    /redoc        - Alternative API docs")
...
def _create_api_app(
    ...
    class PredictionRequest(BaseModel):
        ...
        return_probabilities: bool = Field(
            False, description="Return class probabilities"
        )
    ...
    async def predict_batch(
        ...
        try:
            ...
            return BatchPredictionResponse(
                predictions=predictions, total_time=time.time() - start
            )
