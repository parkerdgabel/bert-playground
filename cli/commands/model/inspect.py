...
from ...utils import (
    ...
    validate_path, create_table, format_bytes
)
...
def inspect_command(
    ...
    component: Optional[str] = typer.Option(None, "--component", "-C",
                                          help="Specific component to inspect"),
    layer: Optional[int] = typer.Option(None, "--layer", "-l",
                                      help="Specific layer number to inspect"),
    show_weights: bool = typer.Option(False, "--weights", "-w",
                                    help="Show weight statistics"),
    show_architecture: bool = typer.Option(True, "--arch/--no-arch",
                                         help="Show model architecture"),
    show_config: bool = typer.Option(True, "--config/--no-config",
                                   help="Show model configuration"),
    show_memory: bool = typer.Option(False, "--memory", "-m",
                                   help="Show memory usage estimates"),
    show_computation: bool = typer.Option(False, "--computation",
                                        help="Show computational complexity"),
    export_json: Optional[Path] = typer.Option(None, "--export", "-e",
                                             help="Export inspection to JSON"),
    format: str = typer.Option("tree", "--format", "-f",
                             help="Display format: tree, table, json"),
...
def _display_architecture(info: dict, component: Optional[str], 
                         layer: Optional[int], format: str):
    ...
    console.print(f"Trainable parameters: [green]{arch['trainable_parameters']:,}[/green]")
    console.print()
...
def _display_architecture_tree(arch: dict, component: Optional[str], layer: Optional[int]):
        ...
        if comp_name in ['encoder', 'bert', 'roberta']:
            for layer_name, layer_info in arch['layers'].items():
                ...
                if len(layer_info['weights']) > 5:
                    layer_node.add(f"[dim]... and {len(layer_info['weights']) - 5} more[/dim]")
    
    console.print(tree)


def _display_architecture_table(arch: dict, component: Optional[str], layer: Optional[int]):
...
def _display_weights(info: dict, component: Optional[str], 
                    layer: Optional[int], verbose: bool):
    """Display weight statistics."""
...
def _display_memory(info: dict):
    ...
    for batch_size in batch_sizes:
        for seq_length in seq_lengths:
            ...
            activation_size = (
                batch_size * seq_length * hidden_size * 4 *  # embeddings
...
def _display_computation(info: dict):
    ...
    operations = {
        ...
        "Attention (per layer)": 4 * hidden_size * hidden_size + 2 * hidden_size * hidden_size,
        "FFN (per layer)": 2 * hidden_size * intermediate_size + 2 * intermediate_size * hidden_size,
    ...
    table.add_row("[bold]Total[/bold]", f"[bold]{total_flops:,}[/bold]", "[bold]100.0%[/bold]")
