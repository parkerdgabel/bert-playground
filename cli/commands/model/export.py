...
def export_command(
    ...
    format: str = typer.Option("mlx", "--format", "-f", 
                             help="Export format: mlx, onnx, coreml, safetensors"),
    model_name: Optional[str] = typer.Option(None, "--name", "-n",
                                           help="Name for the exported model"),
    quantize: bool = typer.Option(False, "--quantize", "-q",
                                help="Quantize model (format-specific)"),
    bits: int = typer.Option(8, "--bits", "-b",
                           help="Quantization bits (4, 8, or 16)"),
    optimize: bool = typer.Option(True, "--optimize/--no-optimize",
                                help="Apply format-specific optimizations"),
    include_tokenizer: bool = typer.Option(True, "--tokenizer/--no-tokenizer",
                                         help="Include tokenizer with export"),
    ...
    if format.lower() not in supported_formats:
        print_error(f"Unsupported format: {format}. Supported: {', '.join(supported_formats)}")
        raise typer.Exit(1)
    
    # Check if output exists
    if output_path.exists() and not force:
        print_error(f"Output path already exists: {output_path}\nUse --force to overwrite")
        raise typer.Exit(1)
    ...
    try:
        if format == "mlx":
            _export_mlx(checkpoint, output_path, export_metadata, include_tokenizer, verbose)
        
        elif format == "onnx":
            _export_onnx(checkpoint, output_path, export_metadata, quantize, bits, optimize, verbose)
        
        elif format == "coreml":
            _export_coreml(checkpoint, output_path, export_metadata, quantize, optimize, verbose)
...
def _export_mlx(checkpoint: Path, output_path: Path, metadata: dict, 
                include_tokenizer: bool, verbose: bool):
...
def _export_onnx(checkpoint: Path, output_path: Path, metadata: dict,
                 quantize: bool, bits: int, optimize: bool, verbose: bool):
...
def _export_coreml(checkpoint: Path, output_path: Path, metadata: dict,
                   quantize: bool, optimize: bool, verbose: bool):
...
def _export_safetensors(checkpoint: Path, output_path: Path, metadata: dict, verbose: bool):
