...
def evaluate_command(
    ...
    num_workers: int = typer.Option(
        4, "--workers", "-w", help="Number of data loading workers"
    ),
    output: Path | None = typer.Option(
        None, "--output", "-o", help="Save evaluation results to file"
    ),
    save_predictions: bool = typer.Option(
        False, "--save-predictions", "-p", help="Save all predictions to CSV"
    ),
    confusion_matrix: bool = typer.Option(
        False, "--confusion-matrix", help="Generate confusion matrix"
    ),
    per_class: bool = typer.Option(False, "--per-class", help="Show per-class metrics"),
    error_analysis: bool = typer.Option(
        False, "--error-analysis", "-e", help="Perform error analysis"
    ),
    mlflow_run: str | None = typer.Option(
        None, "--mlflow-run", help="Log results to MLflow run"
    ),
    compare_with: Path | None = typer.Option(
        None, "--compare", "-C", help="Compare with another model"
    ),
    device: str = typer.Option(
        "auto", "--device", "-d", help="Device to use: auto, cpu, gpu"
    ),
    verbose: bool = typer.Option(
        False, "--verbose", "-v", help="Show detailed evaluation information"
    ),
):
    ...
    try:
        ...
        if save_predictions:
            pred_path = (
                output.parent / f"{output.stem}_predictions.csv"
                if output
                else Path("predictions.csv")
            )
        ...
        if compare_with:
            _compare_models(
                checkpoint, compare_with, test_dataset, metrics, batch_size, max_length
            )
...
def _load_test_data(
    test_file: Path, max_length: int, verbose: bool
) -> list[dict[str, Any]]:
...
def _evaluate_model(
    ...
    if "precision" in metrics:
        computed_metrics["precision_macro"] = precision_score(
            y_true, y_pred, average="macro"
        )
        computed_metrics["precision_weighted"] = precision_score(
            y_true, y_pred, average="weighted"
        )

    if "recall" in metrics:
        computed_metrics["recall_macro"] = recall_score(y_true, y_pred, average="macro")
        computed_metrics["recall_weighted"] = recall_score(
            y_true, y_pred, average="weighted"
        )
...
def _perform_error_analysis(
    results: dict[str, Any], dataset: list[dict[str, Any]], config: dict
):
    ...
    for i, (pred, true) in enumerate(
        zip(results["predictions"], results["labels"], strict=False)
    ):
        if pred != true:
            errors.append(
                {
                    "id": results["ids"][i],
                    "text": results["texts"][i][:100] + "..."
                    if len(results["texts"][i]) > 100
                    else results["texts"][i],
                    "true_label": true,
    ...
    console.print(
        f"Total errors: {len(errors)} / {len(results['predictions'])} ({len(errors) / len(results['predictions']) * 100:.1f}%)"
    )
    ...
    if errors:
        ...
        for error in errors[:10]:
            ...
            pred_label = label_map.get(
                error["predicted_label"], str(error["predicted_label"])
            )
...
def _save_predictions(
    results: dict[str, Any], dataset: list[dict[str, Any]], output_path: Path
):
...
def _compare_models(
    ...
    for metric in results1["metrics"]:
        ...
        table.add_row(
            metric.replace("_", " ").title(), f"{val1:.4f}", f"{val2:.4f}", diff_str
        )
...
def _save_results(
    ...
    save_data = {
        ...
        "evaluation_settings": {"metrics": metrics},
    }
