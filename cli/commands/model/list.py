...
def list_models_command(
    path: Path = typer.Option(
        Path("output"), "--path", "-p", help="Directory to search for models"
    ),
    format: str = typer.Option(
        "table", "--format", "-f", help="Output format: table, json, simple"
    ),
    sort_by: str = typer.Option(
        "date", "--sort", "-s", help="Sort by: date, size, name, accuracy"
    ),
    reverse: bool = typer.Option(False, "--reverse", "-r", help="Reverse sort order"),
    filter_name: str | None = typer.Option(
        None, "--name", "-n", help="Filter by model name pattern"
    ),
    filter_type: str | None = typer.Option(
        None, "--type", "-t", help="Filter by model type"
    ),
    show_metrics: bool = typer.Option(
        True, "--metrics/--no-metrics", help="Show model metrics"
    ),
    show_config: bool = typer.Option(
        False, "--config", "-c", help="Show model configuration"
    ),
    limit: int = typer.Option(
        0, "--limit", "-l", help="Limit number of results (0 for all)"
    ),
    recursive: bool = typer.Option(
        True, "--recursive/--no-recursive", help="Search recursively"
    ),
    include_mlflow: bool = typer.Option(
        False, "--mlflow", "-m", help="Include MLflow tracked models"
    ),
...
def _find_models(
    ...
    for search_path in search_paths:
        ...
        is_model = any(
            (search_path / indicator).exists() for indicator in model_indicators
        )
...
def _sort_models(models: list[dict], sort_by: str, reverse: bool) -> list[dict]:
    ...
    if sort_by == "date":
        ...
        models.sort(
            key=lambda m: m["metrics"].get("val_accuracy", 0)
            or m["metrics"].get("accuracy", 0),
            reverse=not reverse,
        )
...
def _display_table(models: list[dict], show_metrics: bool, show_config: bool):
    ...
    for model in models:
        ...
        if show_metrics:
            accuracy = model["metrics"].get("val_accuracy") or model["metrics"].get(
                "accuracy"
            )
