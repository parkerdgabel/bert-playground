...
def predict_command(
    ...
    output: Path = typer.Option(
        "submission.csv", "--output", "-o", help="Output file path"
    ),
    ...
    dataset_name: str = typer.Option(
        "titanic", "--dataset", help="Dataset name for loader"
    ),
    num_workers: int = typer.Option(
        4, "--workers", help="Number of data loading workers"
    ),
    no_header: bool = typer.Option(
        False, "--no-header", help="Exclude header from output"
    ),
    ...
    if config_path.exists():
        ...
        console.print(
            "[yellow]Warning: No training config found, using defaults[/yellow]"
        )
    ...
    console.print("[green]✓ Created test data loader[/green]")
    ...
    with console.status("[yellow]Generating predictions...[/yellow]"):
        for batch_idx, batch in enumerate(test_loader):
            ...
            # Debug outputs
            logger.debug(
                f"Batch {batch_idx} outputs keys: {outputs.keys() if isinstance(outputs, dict) else 'not a dict'}"
            )
    ...
    if format == "csv":
        ...
        if dataset_name == "titanic":
            submission = pd.DataFrame(
                {"PassengerId": df["PassengerId"], "Survived": predictions[: len(df)]}
            )
        else:
            # Generic format
            submission = pd.DataFrame(
                {"id": range(len(predictions)), "prediction": predictions}
            )
