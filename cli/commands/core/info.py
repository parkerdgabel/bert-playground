...
from ...utils import (
    get_console, print_info, print_error,
    handle_errors
)
...
def info_command(
    ...
    embeddings: bool = typer.Option(False, "--embeddings", help="Show embeddings information"),
...
def _show_system_info(console):
    ...
    try:
        ...
        info_table.add_row("Available Memory", f"{mem.available / 1e9:.1f} GB ({mem.percent:.1f}% used)")
...
def _show_mlx_info(console):
    ...
    # MLX settings
    try:
        mlx_table.add_row("Metal Device", "Available" if mx.metal.is_available() else "Not available")
    except:
...
def _show_models_info(console):
    ...
    try:
        ...
        heads_table = create_table("Available Heads", ["Head Type", "Competition Types", "Description"])
...
def _show_datasets_info(console):
    ...
    try:
        ...
        if datasets:
            datasets_table = create_table("Available Datasets", ["Name", "Task Type", "Features", "Samples"])
...
def _show_embeddings_info(console):
    ...
    try:
        ...
        embeddings_table = create_table("MLX Embeddings Models", ["HuggingFace Model", "MLX Model"])
