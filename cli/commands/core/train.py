...
def train_command(
    ...
    model_type: str = typer.Option(
        "base", "--model-type", help="Model type: base or mlx_embeddings"
    ),
    model_name: str = typer.Option(
        "answerdotai/ModernBERT-base", "--model", "-m", help="Model name or path"
    ),
    pretrained: str | None = typer.Option(
        None, "--pretrained", "-p", help="Pretrained model name or path"
    ),
    # Training arguments
    epochs: int = typer.Option(
        5, "--epochs", "-e", help="Number of training epochs", callback=validate_epochs
    ),
    ...
    learning_rate: float = typer.Option(
        2e-5, "--lr", "-l", help="Learning rate", callback=validate_learning_rate
    ),
    warmup_ratio: float = typer.Option(0.1, "--warmup-ratio", help="Warmup ratio"),
    warmup_steps: int | None = typer.Option(
        None, "--warmup-steps", "-w", help="Number of warmup steps (overrides ratio)"
    ),
    # Configuration
    ...
    kaggle: bool = typer.Option(
        False, "--kaggle", help="Use KaggleTrainer for competition optimization"
    ),
    competition: str | None = typer.Option(
        None, "--competition", help="Kaggle competition name (e.g., titanic)"
    ),
    cv_folds: int = typer.Option(
        5, "--cv-folds", help="Number of CV folds for KaggleTrainer"
    ),
    # Output arguments
    ...
    use_mlx_embeddings: bool = typer.Option(
        False, "--mlx-embeddings", help="Use MLX embeddings"
    ),
    ...
    mixed_precision: bool = typer.Option(
        True, "--mixed-precision/--no-mixed-precision", help="Use mixed precision"
    ),
    ...
    prefetch_size: int = typer.Option(
        4, "--prefetch", help="Data prefetch size (0 to disable)"
    ),
    use_pretokenized: bool = typer.Option(
        False, "--pretokenize", help="Pre-tokenize data for optimal performance"
    ),
    # Training control
    ...
    logging_steps: int = typer.Option(
        10, "--logging-steps", help="Progress logging frequency"
    ),
    save_best_only: bool = typer.Option(
        False, "--save-best-only", help="Save only the best model"
    ),
    # Advanced options
    ...
    use_compilation = config_overrides.get("training", {}).get(
    ...
    if kaggle:
        ...
        training_config.environment.run_name = (
            run_name or f"kaggle_{competition}_{timestamp}"
        )
    ...
    try:
        ...
        if kaggle:
            ...
            if (
                test_loader
                and hasattr(trainer, "test_predictions")
                and trainer.test_predictions is not None
            ):
                logger.info(
