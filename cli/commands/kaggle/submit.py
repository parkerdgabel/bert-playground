...
def submit_command(
    ...
    message: str = typer.Option("Submission from MLX-BERT CLI", "--message", "-m", 
                               help="Submission message (max 512 chars)"),
    checkpoint: Optional[Path] = typer.Option(None, "--checkpoint", "-c", 
                                            help="Associated model checkpoint for tracking"),
    track_mlflow: bool = typer.Option(True, "--track/--no-track", 
                                     help="Track submission in MLflow"),
):
    ...
    if not submission_file.suffix == '.csv':
        print_error(
            "Submission file must be a CSV file",
    ...
    try:
        ...
        submission_table.add_row("File Size", f"{submission_file.stat().st_size / 1024:.1f} KB")
                ...
                if checkpoint_info_path.exists():
                    ...
                    if run_id:
                        with mlflow.start_run(run_id=run_id):
                            # Log submission info
                            mlflow.log_param(f"kaggle_submission_{datetime.now().strftime('%Y%m%d_%H%M%S')}", {
        ...
        console.print(f"1. View submission status: [cyan]bert kaggle history {competition}[/cyan]")
        console.print(f"2. Check leaderboard: [cyan]bert kaggle leaderboard {competition}[/cyan]")
...
def auto_submit_command(
    ...
    config: Optional[Path] = typer.Option(None, "--config", "-c", 
                                        help="Competition config file"),
    message_template: str = typer.Option("Auto-submission: {model_name} - Score: {val_score:.4f}", 
                                       "--template", help="Message template with placeholders"),
    batch_size: int = typer.Option(64, "--batch-size", "-b", help="Prediction batch size"),
    output_dir: Path = typer.Option("submissions", "--output", "-o", help="Output directory"),
):
    ...
    try:
        ...
        info_table.add_row("Best Score", f"{checkpoint_info.get('best_val_accuracy', 0):.4f}")
        info_table.add_row("Epoch", str(checkpoint_info.get("epoch", "Unknown")))
        info_table.add_row("Learning Rate", str(training_config.get("learning_rate", "Unknown")))
        ...
        print_success(f"Successfully submitted to Kaggle!")
        ...
        # Show next steps
        console.print("\n[bold]Next steps:[/bold]")
        console.print(f"1. Check submission status: [cyan]bert kaggle history {competition}[/cyan]")
        console.print(f"2. View leaderboard: [cyan]bert kaggle leaderboard {competition}[/cyan]")
