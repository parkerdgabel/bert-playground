...
from ...utils import get_console, handle_errors, print_error, print_info, print_success
...
def history_command(
    ...
    limit: int = typer.Option(
        10, "--limit", "-l", help="Number of submissions to show"
    ),
    report: Path | None = typer.Option(
        None, "--report", "-r", help="Generate detailed report (JSON)"
    ),
    show_scores: bool = typer.Option(
        True, "--scores/--no-scores", help="Show public/private scores"
    ),
    show_files: bool = typer.Option(
        False, "--files", help="Show submission file names"
    ),
):
    ...
    try:
        ...
        hist_table = create_table(
            f"Last {min(limit, len(submissions))} Submissions", columns
        )
            ...
            if first_date and last_date:
                try:
                    ...
                    if days > 0:
                        ...
                        console.print(
                            f"[cyan]Submission Frequency: {freq:.2f} per day[/cyan]"
                        )
                except:
                    pass
        ...
        console.print(
            f"1. View leaderboard position: [cyan]bert kaggle leaderboard {competition}[/cyan]"
        )
        console.print(
            f"2. Make new submission: [cyan]bert kaggle submit {competition} predictions.csv[/cyan]"
        )
        console.print(
            f"3. Download best submission: [cyan]bert kaggle download {competition} --file submission.csv[/cyan]"
        )

    except Exception as e:
        print_error(
            f"Failed to fetch submission history: {str(e)}", title="History Error"
        )
        raise typer.Exit(1)
