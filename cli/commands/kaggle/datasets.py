...
def datasets_command(
    search: Optional[str] = typer.Option(None, "--search", "-s", help="Search datasets by keyword"),
    tag: Optional[str] = typer.Option(None, "--tag", "-t", help="Filter by tag (e.g., 'nlp', 'tabular')"),
    user: Optional[str] = typer.Option(None, "--user", "-u", help="Filter by username"),
    sort_by: str = typer.Option("votes", "--sort", help="Sort by: votes, updated, size, relevance"),
    limit: int = typer.Option(20, "--limit", "-l", help="Number of datasets to show"),
    file_type: Optional[str] = typer.Option(None, "--file-type", help="Filter by file type (csv, json, etc.)"),
    min_size: Optional[int] = typer.Option(None, "--min-size", help="Minimum size in MB"),
    max_size: Optional[int] = typer.Option(None, "--max-size", help="Maximum size in MB"),
):
    ...
    try:
        ...
        ds_table = create_table("Available Datasets", 
        ...
        console.print("  • Download a dataset: [cyan]bert kaggle download-dataset DATASET_REF[/cyan]")
        console.print("  • Create data loader for dataset: [cyan]bert model create-loader --dataset DATASET_REF[/cyan]")
        console.print("  • View dataset on Kaggle: [cyan]https://kaggle.com/datasets/DATASET_REF[/cyan]")
