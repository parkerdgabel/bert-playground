...
def leaderboard_command(
    ...
    save: Path | None = typer.Option(
        None, "--save", "-s", help="Save leaderboard to file (CSV or JSON)"
    ),
    show_change: bool = typer.Option(False, "--changes", help="Show position changes"),
    highlight_user: bool = typer.Option(
        True, "--highlight/--no-highlight", help="Highlight your submissions"
    ),
):
    ...
    try:
        ...
        lb_table = create_table(
            f"Top {min(top_n, len(leaderboard))} Leaderboard", columns
        )
        ...
        if save:
            if save.suffix == ".csv":
                ...
                print_warning(
                    f"Unsupported file format: {save.suffix}. Use .csv or .json"
                )
                return
        ...
        console.print(
            f"1. View your submissions: [cyan]bert kaggle history {competition}[/cyan]"
        )
        console.print(
            f"2. Make a new submission: [cyan]bert kaggle submit {competition} predictions.csv[/cyan]"
        )
        console.print(
            f"3. Download competition data: [cyan]bert kaggle download {competition}[/cyan]"
        )
