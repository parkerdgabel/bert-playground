...
def download_competition_command(
    ...
    unzip: bool = typer.Option(
        True, "--unzip/--no-unzip", help="Unzip downloaded files"
    ),
    specific_file: str | None = typer.Option(
        None, "--file", "-f", help="Download specific file only"
    ),
    force: bool = typer.Option(
        False, "--force", help="Force re-download even if files exist"
    ),
):
    ...
    try:
        ...
        output_path = kaggle.download_competition_data(
            competition_id=competition, path=output_dir, unzip=unzip
        )
        ...
        console.print(
            f"2. Train a model: [cyan]bert train --train {output_path}/train.csv[/cyan]"
        )
        console.print(
            f"3. View competition info: [cyan]bert kaggle leaderboard {competition}[/cyan]"
        )

    except Exception as e:
        print_error(
            f"Failed to download competition data: {str(e)}", title="Download Error"
        )
        raise typer.Exit(1)
...
def download_dataset_command(
    dataset: str = typer.Argument(
        ..., help="Dataset identifier (e.g., username/dataset-name)"
    ),
    output_dir: Path | None = typer.Option(
        None, "--output", "-o", help="Output directory"
    ),
    ...
    try:
        ...
        output_path = dataset_manager.download_dataset(
            dataset_ref=dataset, output_dir=output_dir, unzip=unzip
        )
        ...
        console.print("\n[bold]Next steps:[/bold]")
        console.print(f"1. Explore the data: [cyan]ls {output_path}[/cyan]")
        console.print("2. Create a data loader for the dataset")
        console.print("3. Train a model with the data")
