...
def competitions_command(
    category: str | None = typer.Option(
        ...
        "--category",
        "-c",
        help="Filter by category (e.g., 'tabular', 'nlp', 'cv')",
    ),
    search: str | None = typer.Option(
        None, "--search", "-s", help="Search competitions by keyword"
    ),
    sort_by: str = typer.Option(
        "latestDeadline",
        "--sort",
        help="Sort by: latestDeadline, prize, numberOfTeams, recentlyCreated",
    ),
    limit: int = typer.Option(
        20, "--limit", "-l", help="Number of competitions to show (max: 50)"
    ),
    active_only: bool = typer.Option(
        True, "--active/--all", help="Show only active competitions"
    ),
    ...
    try:
        ...
        if active_only:
            competitions_list = [
                c for c in competitions_list if not c.get("isCompleted", True)
            ]
            if not competitions_list:
                print_info(
                    "No active competitions found. Use --all to see completed ones."
                )
                return
        ...
        console.print(
            "  • Download competition data: [cyan]bert kaggle download COMPETITION_ID[/cyan]"
        )
        console.print(
            "  • View leaderboard: [cyan]bert kaggle leaderboard COMPETITION_ID[/cyan]"
        )
        console.print(
            "  • Submit predictions: [cyan]bert kaggle submit COMPETITION_ID submission.csv[/cyan]"
        )
