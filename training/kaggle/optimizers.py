...
class LayerWiseAdamW:
    ...
    def __init__(
        ...
        logger.info(f"Created LayerWiseAdamW with {len(self.optimizers)} parameter groups")
        for opt_dict in self.optimizers:
            logger.debug(f"  Group '{opt_dict['name']}': {len(opt_dict['params'])} parameters, "
    ...
    def learning_rate(self):
        ...
        if not self.optimizers:
            return 0.0
        return sum(opt['optimizer'].learning_rate for opt in self.optimizers) / len(self.optimizers)
