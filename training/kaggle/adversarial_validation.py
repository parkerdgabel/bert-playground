...
class AdversarialValidationConfig:
    ...
    # Sampling settings
    sample_size: Optional[int] = None  # Sample size for large datasets
...
class AdversarialValidator:
    ...
    def compute_distribution_similarity(
        ...
        if self.config.sample_size and len(combined_data) > self.config.sample_size:
            combined_data = combined_data.sample(
                ...
                stratify=combined_data['is_test'] if self.config.stratify_sampling else None,
                random_state=42
            )
        
        # Get features and labels
        X = combined_data[text_column] if text_column in combined_data.columns else combined_data
        ...
        if self.similarity_score < self.config.similarity_threshold:
            ...
            logger.warning(f"⚠ Distribution shift detected (ROC-AUC: {self.similarity_score:.4f})")
    ...
    def create_adversarial_validation_split(
        ...
        for df in [new_train_data, val_data]:
            df.drop(['is_test', 'test_probability'], axis=1, inplace=True, errors='ignore')
        
        logger.info(f"Created validation set with {len(val_data)} samples (most similar to test)")
...
def create_leak_detection_features(
    ...
    if id_column in train_data.columns and id_column in test_data.columns:
        ...
        if overlap:
            ...
            logger.warning(f"Found {len(overlap)} overlapping IDs between train and test")
