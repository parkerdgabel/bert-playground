...
class PseudoLabelingConfig:
    ...
    # Label selection
    select_top_k: Optional[int] = None  # Select top K most confident
...
class BERTConfidenceScorer:
    ...
    def compute_confidence(
        ...
        token_logits: Optional[mx.array] = None
    ) -> mx.array:
        """
        ...
        if attention_weights is not None and self.config.use_attention_confidence:
        ...
        if len(confidences) > 1:
            # Weighted average
            weights = mx.array([0.4, 0.3] + [0.3 / (len(confidences) - 2)] * (len(confidences) - 2))
            confidence = mx.sum(mx.stack(confidences) * weights[:len(confidences), None], axis=0)
        else:
    ...
    def _compute_attention_confidence(self, attention_weights: mx.array) -> mx.array:
        ...
        attention_entropy = -mx.sum(
            avg_attention * mx.log(avg_attention + 1e-10), 
            axis=-1
        )  # [batch_size, seq_len]
...
class BERTPseudoLabeler:
    ...
    def __init__(self, config: PseudoLabelingConfig):
        ...
        """
        self.config = config
        self.confidence_scorer = BERTConfidenceScorer(config)
        self.iteration = 0
    ...
    def generate_pseudo_labels(
        ...
        logger.info(f"Selected {len(pseudo_labeled_samples)} confident samples "
    ...
    def iterative_pseudo_labeling(
        ...
        for iteration in range(self.config.num_iterations):
            logger.info(f"Pseudo-labeling iteration {iteration + 1}/{self.config.num_iterations}")
    ...
    def _create_augmented_loader(
        ...
        logger.info(f"Created augmented dataset with {len(pseudo_samples)} pseudo-labels")
...
class EnsemblePseudoLabeler(BERTPseudoLabeler):
    ...
    def generate_pseudo_labels(
        ...
        for model_idx, model in enumerate(self.models):
            logger.info(f"Getting predictions from model {model_idx + 1}/{len(self.models)}")
