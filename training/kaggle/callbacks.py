...
class KaggleSubmissionCallback(Callback):
    ...
    def _check_kaggle_api(self):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning("Kaggle API not configured. Please run 'kaggle config set'")
                self.enabled = False
            else:
                self.enabled = True
    ...
    def _prepare_and_submit(self, trainer: Trainer, state: TrainingState, is_final: bool = False):
...
class CompetitionMetrics(Callback):
    ...
    def on_evaluate_end(self, trainer: Trainer, state: TrainingState, metrics: Dict[str, float]) -> None:
        ...
        if self.log_improvement and is_improvement:
            improvement = abs(current_score - self.scores_history[-2][1]) if len(self.scores_history) > 1 else 0
            logger.info(
                f"Competition metric improved! {self.metric_name}: {current_score:.4f} "
                f"(+{improvement:.4f})" if self.maximize else f"(-{improvement:.4f})"
            )
    ...
    def on_train_end(self, trainer: Trainer, state: TrainingState, result: TrainingResult) -> None:
        ...
        logger.info(f"Final Score: {self.scores_history[-1][1]:.4f}" if self.scores_history else "N/A")
        
        # Save history
        if self.scores_history:
            history_df = pd.DataFrame(self.scores_history, columns=["epoch", self.metric_name])
            history_path = trainer.config.environment.output_dir / f"competition_metric_{self.metric_name}.csv"
