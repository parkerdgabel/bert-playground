    ...
    **kwargs,
    ...
    # Use preset if name provided
        ...
        if config_name not in CONFIG_REGISTRY:
            raise ValueError(
                f"Unknown config: {config_name}. Available: {', '.join(list_configs())}"
            )
...
def create_trainer(
    ...
    config: BaseTrainerConfig | str | Path | None = None,
...
def _get_default_callbacks(config: BaseTrainerConfig) -> list[Callback]:
    ...
    if progress_config.get("show_batch_progress", True) or progress_config.get(
...
def _apply_config_overrides(
    config: BaseTrainerConfig, overrides: dict[str, Any]
) -> None:
