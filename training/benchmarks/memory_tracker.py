...
class MemoryTracker:
    ...
    def log_stats(self):
        ...
        logger.info(f"Memory - Current: {stats['current_gb']:.2f}GB, Peak: {stats['peak_gb']:.2f}GB")
        if "used_gb" in stats:
            logger.info(f"Memory - Used: {stats['used_gb']:.2f}GB, Peak Used: {stats['peak_used_gb']:.2f}GB")
    ...
    def track_operation(self, operation: Callable, operation_name: str) -> Any:
        ...
        """
        # Memory before
        before_memory = mx.metal.get_active_memory() / (1024**3) if hasattr(mx, 'metal') else 0
        ...
        # Memory after
        after_memory = mx.metal.get_active_memory() / (1024**3) if hasattr(mx, 'metal') else 0
    ...
    def check_memory_pressure(self, threshold_gb: float = 0.8) -> bool:
        ...
        try:
            ...
            if self.current_memory > 50:  # Over 50GB suggests high usage
                logger.warning(f"High memory usage detected: {self.current_memory:.1f}GB")
                return True
