...
from training.benchmarks import MemoryTracker, PerformanceMonitor
...
class DummyModel(nn.Module):
    ...
    def __init__(
        self, input_dim: int = 768, hidden_dim: int = 256, num_classes: int = 2
    ):
        super().__init__()
...
def benchmark_full_training():
    ...
    logger.info(
        f"Average time per epoch: {total_time / config.training.num_epochs:.2f} seconds"
    )
    logger.info(
        f"Throughput: {(train_loader.num_samples * config.training.num_epochs) / total_time:.1f} samples/sec"
    )
