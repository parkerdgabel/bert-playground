...
from training.benchmarks import MemoryTracker
from training.benchmarks.test_optimizations import DummyDataLoader, DummyModel
...
def benchmark_gradient_stats():
    ...
    logger.info(
        f"Optimized (norm only): {optimized_time:.3f}s ({optimized_time / num_iterations * 1000:.2f}ms per call)"
    )
    logger.info(
        f"Detailed stats: {detailed_time:.3f}s ({detailed_time / num_iterations * 1000:.2f}ms per call)"
    )
...
def benchmark_full_training_phase2():
    ...
    logger.info(
        f"With gradient accumulation: effective batch size = {config.data.batch_size * config.training.gradient_accumulation_steps}"
    )
