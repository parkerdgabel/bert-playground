...
class BaseTrainer:
        ...
        def train_step(batch: dict[str, mx.array]) -> tuple[float, dict[str, mx.array]]:
            ...
            if self.config.optimizer.max_grad_norm > 0:
                grads, grad_norm = clip_gradients(
                    grads, self.config.optimizer.max_grad_norm
                )
            else:
    ...
    def train(
        ...
        for epoch in range(self.state.epoch, self.config.training.num_epochs):
            ...
            should_evaluate = val_dataloader is not None and (
                self.config.training.eval_strategy == "epoch"
                or (epoch + 1) == self.config.training.num_epochs
            )
            ...
            if should_evaluate:
                ...
                if self.config.training.early_stopping:
                    if (
                        ...
                        logger.info(
                            f"Early stopping triggered after {self.state.no_improvement_count} epochs without improvement"
                        )

            # Save checkpoint if needed
            should_save = self.config.training.save_strategy == "epoch" or (
        ...
        result = TrainingResult(
            ...
            early_stopped=self.state.should_stop
            and self.config.training.early_stopping,
        ...
        def make_json_serializable(obj):
            ...
            if isinstance(obj, mx.array) or isinstance(obj, np.ndarray):
                try:
    ...
    def _train_epoch(self, dataloader: DataLoader, epoch: int) -> dict[str, float]:
        ...
        avg_loss = (
            float(epoch_loss.item()) / num_batches
            if hasattr(epoch_loss, "item")
            else float(epoch_loss) / num_batches
        )
    ...
    def _call_hooks(self, method: str, *args, **kwargs) -> None:
        ...
        if method == "on_batch_begin":
            logger.debug(
                f"Calling hooks for {method}, num callbacks: {len(self.callbacks)}"
            )
