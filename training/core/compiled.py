...
def create_compiled_train_step(
    ...
    config: Any,
    gradient_accumulator: Any | None = None,
    ...
    def loss_fn(model, batch):
        ...
        model_inputs = {
            k: v for k, v in batch.items() if k not in ["metadata"] and v is not None
        }
    ...
    def compiled_train_step(
        ...
        if (
            ...
            and config.training.gradient_accumulation_steps > 1
        ):
            ...
            grads = _scale_gradients(
                grads, 1.0 / config.training.gradient_accumulation_steps
            )
...
def create_compiled_eval_step(model: Model) -> callable:
    ...
    def compiled_eval_step(
        ...
        if loss is None:
            raise ValueError(
                f"Model must return a dictionary with 'loss' key, got keys: {list(outputs.keys())}"
            )
...
def _clip_gradients_compiled(grads: dict[str, Any], max_norm: float) -> dict[str, Any]:
    ...
    clipped_grads = tree_map(
        lambda g: g * clip_factor if g is not None else None, grads
    )
...
def should_compile_model(model: Model, config: Any) -> bool:
    ...
    if param_count < 1000:  # Less than 1K parameters
        logger.info(
            f"Model too small for compilation benefits ({param_count} parameters)"
        )
        return False
...
def optimize_batch_size_for_compilation(
    model: Model, current_batch_size: int, config: Any
) -> int:
    """
    ...
    for size in optimal_sizes:
        if size >= current_batch_size:
            if size != current_batch_size:
                logger.info(
                    f"Suggesting batch size {size} (from {current_batch_size}) for better compilation performance"
                )
            return size
