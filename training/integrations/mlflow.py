...
from ..core.protocols import Model, TrainingState, TrainingResult
...
class MLflowIntegration:
    ...
    def __init__(self, config: MLflowConfig):
        ...
        logger.info(f"Initialized MLflow integration with tracking URI: {mlflow.get_tracking_uri()}")
    
    @contextmanager
    def start_run(self, trainer_config: Optional[BaseTrainerConfig] = None):
        """
    ...
    def log_training_state(self, state: TrainingState, step: Optional[int] = None) -> None:
        """
    ...
    def log_training_result(self, result: TrainingResult) -> None:
        ...
        summary = {
            ...
            "best_model_path": str(result.best_model_path) if result.best_model_path else None,
        }
    ...
    def _log_trainer_config(self, config: BaseTrainerConfig) -> None:
        ...
        # Log data config
        data_params = {
            f"data.{k}": v for k, v in config.data.to_dict().items()
        }
