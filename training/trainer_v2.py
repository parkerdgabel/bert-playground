...
class TitanicTrainerV2:
    ...
    def __init__(
        ...
        logger.info(
            "Initialized TitanicTrainerV2 with enhanced logging and MLflow tracking"
        )
    ...
    def train_step(
        ...
        if self.global_step % 100 == 0:
            logger.debug(
                f"Batch shape: input_ids={batch['input_ids'].shape}, "
        ...
        if self.global_step % 50 == 0 and grads:
            ...
            if grad_norms:
                logger.debug(
                    f"Gradient norms at step {self.global_step}: "
    ...
    @log_execution_time
    def evaluate(
        self, dataloader: TitanicDataPipeline, phase: str = "val"
    ) -> Dict[str, float]:
        ...
        if all_labels:
            ...
            if self.enable_mlflow and phase == "val":
                ...
                self.mlflow_tracker.log_roc_curve(
                    np.array(all_labels), np.array(all_probs), step=self.global_step
                )
    ...
    def train(self, num_epochs: int):
        ...
        with ExperimentLogger(
            ...
            logger.info(
                f"Starting training for {num_epochs} epochs ({self.max_steps} steps)"
            )
            ...
            for epoch in range(num_epochs):
                ...
                for batch_idx, batch in enumerate(self.train_loader.get_dataloader()()):
                    try:
                            ...
                            if val_metrics and "val_loss" in val_metrics:
                                if val_metrics["val_loss"] < self.best_val_loss:
                                    ...
                                    self.save_checkpoint(
                                        "best_model_loss", is_best=True
                                    )
                                    logger.success(
                                        f"New best model (loss) saved at step {self.global_step}"
                                    )

                                if val_metrics["val_accuracy"] > self.best_val_accuracy:
                                    self.best_val_accuracy = val_metrics["val_accuracy"]
                                    self.save_checkpoint(
                                        "best_model_accuracy", is_best=True
                                    )
                                    logger.success(
                                        f"New best model (accuracy) saved at step {self.global_step}"
                                    )
                            else:
                                logger.warning(
                                    f"Validation metrics empty at step {self.global_step}"
                                )

                        # Periodic checkpoint
                        if self.global_step % self.save_steps == 0:
                            self.save_checkpoint(
                                f"checkpoint_{self.global_step}", is_best=False
                            )
                        ...
                        logger.error(
                            f"Error in training step {self.global_step}: {str(e)}"
                        )
    ...
    def save_checkpoint(self, name: str, is_best: bool = False):
        ...
        try:
            ...
            mx.save_safetensors(str(checkpoint_dir / "model.safetensors"), model_params)
            logger.info(
                f"Model weights saved to {checkpoint_dir / 'model.safetensors'}"
            )
        ...
        try:
            ...
            mx.save_safetensors(
                str(checkpoint_dir / "optimizer.safetensors"), optimizer_state
            )
            logger.info(
                f"Optimizer state saved to {checkpoint_dir / 'optimizer.safetensors'}"
            )
        ...
        if self.enable_mlflow and mlflow.active_run():
            try:
                self.mlflow_tracker.log_model(
                    str(checkpoint_dir), name, metadata=metadata
                )
                if is_best:
                    self.mlflow_tracker.log_metrics(
                        {
...
def create_trainer_v2(
    ...
    from data.unified_loader import create_data_loaders
