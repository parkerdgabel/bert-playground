...
class MetricsLogger(Callback):
    ...
    def on_batch_begin(
        self, trainer: Trainer, state: TrainingState, batch: dict[str, Any]
    ) -> None:
    ...
    def on_train_end(
        ...
        summary = {
            ...
            "average_batch_time": sum(self.batch_times) / len(self.batch_times)
            if self.batch_times
            else 0,
        }
    ...
    def _update_plots(self, final: bool = False) -> None:
        ...
        accuracy_metrics = [
            ...
            if any(x in k for x in ["accuracy", "acc", "auc", "f1"])
        ]
        other_metrics = [
            k for k in self.metrics_history if k not in loss_metrics + accuracy_metrics
        ]

        # Create figure with subplots
        num_plots = sum(
            [len(loss_metrics) > 0, len(accuracy_metrics) > 0, len(other_metrics) > 0]
        )
        if num_plots == 0:
            return
        ...
        if loss_metrics:
            ...
            for metric_name in loss_metrics:
                steps, values = zip(*self.metrics_history[metric_name], strict=False)
    ...
    def _compute_aggregates(self) -> dict[str, Any]:
        ...
        for metric_name, history in self.metrics_history.items():
            ...
            _, values = zip(*history, strict=False)
