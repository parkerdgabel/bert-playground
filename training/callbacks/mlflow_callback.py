...
from ..integrations.mlflow import MLflowIntegration, MLflowConfig, create_mlflow_integration
...
class MLflowCallback(Callback):
    ...
    def __init__(
        ...
        self.mlflow: Optional[MLflowIntegration] = None
    ...
    def on_checkpoint_save(self, trainer: Trainer, state: TrainingState, checkpoint_path: str) -> None:
    ...
    def on_train_end(self, trainer: Trainer, state: TrainingState, result: TrainingResult) -> None:
        ...
        try:
            ...
            if self.log_best_model and result.best_model_path:
                ...
                # Load best model
                best_model = trainer.model.__class__.load_pretrained(result.best_model_path)
                
                # Log to MLflow
                self.mlflow.log_model(
                    model=best_model,
                    artifact_path="best_model",
                    registered_model_name=self.mlflow_config.model_name if self.mlflow_config else None,
                    metadata={
            ...
            if self.log_artifacts and trainer.config.environment.output_dir.exists():
                # Log config files
                config_files = list(trainer.config.environment.output_dir.glob("*.yaml"))
                config_files.extend(trainer.config.environment.output_dir.glob("*.json"))
    ...
    def on_log(self, trainer: Trainer, state: TrainingState, logs: Dict[str, Any]) -> None:
