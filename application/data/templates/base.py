...
class Template(ABC):
    ...
    def convert_batch(self, data: Any) -> List[str]:  # pd.DataFrame in adapters
        ...
        """
        texts = []
        # Abstract implementation - adapters will override this
        if hasattr(data, 'iterrows'):  # pandas DataFrame
            ...
            for row in data:
                text = self.convert(row if isinstance(row, dict) else row.to_dict())
                texts.append(text)
        else:
            raise NotImplementedError("Adapter must implement convert_batch for this data type")
        return texts
    ...
    def filter_columns(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if self._is_null(value) and not self.config.include_null_values:
                continue
