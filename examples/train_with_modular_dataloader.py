...
def create_titanic_dataloader(config_path: str = None):
    ...
    if config.dataloader.enable_cache:
        ...
        if config.dataloader.cache_tokenized:
            caches['tokenization'] = TokenizationCache(
                cache_dir=Path(config.dataloader.cache_dir or "./cache") / "tokenization",
    ...
    console.print(f"[yellow]Dataset:[/yellow]")
...
def process_batch(batch_data, converter, tokenizer, caches):
    ...
    if 'tokenization' in caches:
        ...
        for text in batch_texts:
            ...
            if cached is not None:
                ...
                encoding_dict = {k: v[0].tolist() if hasattr(v[0], 'tolist') else v[0] for k, v in encoding.items()}
                # Cache it
    ...
    if encodings and isinstance(encodings[0], dict):
        ...
        raise ValueError(f"Unexpected encoding format: {type(encodings[0]) if encodings else 'empty'}")
...
def main():
    ...
    console.print(f"[yellow]Batch shapes:[/yellow]")
    ...
    console.print(f"[yellow]Sample texts:[/yellow]")
