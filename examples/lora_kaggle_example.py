...
def example_1_basic_lora():
    ...
    logger.info("Model created with LoRA adapters")
...
def example_3_qlora_memory_efficient():
    ...
    if hasattr(
        lora_adapter.lora_modules.get(list(lora_adapter.lora_modules.keys())[0], None),
        "memory_footprint",
    ):
        first_module = list(lora_adapter.lora_modules.values())[0]
        memory_info = first_module.memory_footprint
        logger.info(
            f"Memory savings: {memory_info['compression_ratio']:.1f}x compression"
        )
...
def example_4_kaggle_competition_specific():
    ...
    logger.info(
        f"Auto-selected config: r={lora_adapter.config.r}, alpha={lora_adapter.config.alpha}"
    )
...
def example_6_lora_for_different_tasks():
    ...
    for name, config in KAGGLE_LORA_PRESETS.items():
        logger.info(
            f"  - {name}: r={config.r}, targets={len(config.target_modules)} modules"
        )
