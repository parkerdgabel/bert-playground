...
class ModernBertConfig:
    ...
    def from_pretrained(cls, model_name: str):
        ...
        return cls(
            ...
            hidden_act=getattr(
                hf_config, "hidden_activation", "gelu"
            ),  # ModernBERT uses 'hidden_activation'
            hidden_dropout_prob=getattr(
                hf_config, "mlp_dropout", 0.1
            ),  # ModernBERT uses 'mlp_dropout'
            attention_probs_dropout_prob=getattr(
                hf_config, "attention_dropout", 0.1
            ),  # ModernBERT uses 'attention_dropout'
            ...
            layer_norm_eps=getattr(
                hf_config, "norm_eps", 1e-12
            ),  # ModernBERT uses 'norm_eps'
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def __call__(
        ...
        attention_scores = attention_scores / mx.sqrt(
            mx.array(self.attention_head_size)
        )
...
class ModernBertMLX(nn.Module):
    ...
    def from_pretrained(cls, model_name: str, num_labels: int = 2):
        ...
        print(
            f"Note: Weight loading from {model_name} not fully implemented. Using random initialization."
        )
...
def create_model(
    model_name: str = "answerdotai/ModernBERT-base", num_labels: int = 2
) -> ModernBertMLX:
