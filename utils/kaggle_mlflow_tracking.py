...
class KaggleMLflowTracker:
    ...
    def __init__(self, competition_id: str, experiment_name: Optional[str] = None):
    ...
    def start_competition_run(self, 
                            run_name: str,
    ...
    def log_submission(self,
        ...
        submission_json = Path("/tmp") / f"submission_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        if public_score is not None:
            ...
            mlflow.log_metric("best_kaggle_public_score", public_score)  # MLflow will keep the best
        ...
        if leaderboard_rank is not None:
            ...
            mlflow.log_metric("best_kaggle_rank", -leaderboard_rank)  # Negative for "higher is better"
    ...
    def log_leaderboard_snapshot(self, leaderboard_df: pd.DataFrame, top_n: int = 100):
        ...
        snapshot_path = Path("/tmp") / f"leaderboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        if not leaderboard_df.empty:
            ...
            if 'score' in leaderboard_df.columns:
                mlflow.log_metric("leaderboard_top_score", leaderboard_df['score'].max())
                mlflow.log_metric("leaderboard_median_score", leaderboard_df['score'].median())
    ...
    def create_submission_dashboard(self, output_path: Optional[Path] = None):
        ...
        if output_path:
            ...
            summary = {
                ...
                'best_private_score': dashboard_df['private_score'].max() if 'private_score' in dashboard_df else None,
                'best_rank': dashboard_df['leaderboard_rank'].min() if 'leaderboard_rank' in dashboard_df else None,
    ...
    def log_ensemble_submission(self,
        ...
        for exp in all_experiments:
            ...
            if not runs.empty:
                ...
                comparison_data.append({
                    ...
                    'best_public_score': best_run.get('metrics.kaggle_public_score'),
                    'best_private_score': best_run.get('metrics.kaggle_private_score'),
                    'best_val_accuracy': best_run.get('metrics.val_accuracy'),
                    'total_runs': len(runs),
                    'best_run_id': best_run['run_id']
                })
        
        return pd.DataFrame(comparison_data).sort_values('best_public_score', ascending=False)