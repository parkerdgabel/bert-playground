...
class UnifiedModelEvaluator:
    ...
    def __init__(
        ...
        if enable_mlflow and mlflow_experiment:
            self.mlflow_tracker = UnifiedMLflowTracker(
                experiment_name=mlflow_experiment, base_dir="./mlflow"
            )
    ...
    def load_model_info(self, model_path: Path) -> dict:
        ...
        if config_file.exists():
            with open(config_file) as f:
                ...
                info["config"] = config
                info["model_type"] = (
                    "cnn_hybrid" if "cnn_kernel_sizes" in config else "standard"
                )
    ...
    def generate_submission(
        ...
        submission = pd.DataFrame(
            {"PassengerId": test_df["PassengerId"], "Survived": all_preds}
        )
    ...
    def visualize_results(self, save_path: Path | None = None):
        ...
        if "probabilities" in self.val_results:
            fpr, tpr, _ = roc_curve(
                self.val_results["labels"], self.val_results["probabilities"]
            )
    ...
    def submit_to_kaggle(self, submission_path: str, competition: str = "titanic"):
        ...
        try:
            cmd = [
                ...
                submission_path,
                "-m",
                f"MLX ModernBERT submission {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            ]
            ...
            if result.returncode == 0:
                console.print(
                    "✅ Successfully submitted to Kaggle!", style="bold green"
                )
