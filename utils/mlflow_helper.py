...
class UnifiedMLflowTracker:
    ...
    def log_artifacts(self, artifact_paths: Union[str, List[str]], artifact_path: Optional[str] = None):
    ...
    def decorator(func):
        def wrapper(*args, **inner_kwargs):
            with UnifiedMLflowTracker(experiment_name, **kwargs) as tracker:
                ...
                params.update({f"kwarg_{k}": str(v)[:250] for k, v in inner_kwargs.items()})
...
launch_mlflow_ui = lambda base_dir="./mlflow", port=5000: UnifiedMLflowTracker(
    "temp", base_dir=base_dir
).launch_ui(port=port)