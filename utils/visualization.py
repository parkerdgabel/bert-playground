...
class ExperimentVisualizer:
    ...
    def plot_training_history(
        ...
        if 'val_precision' in history and 'val_recall' in history:
            ax.plot(history['val_precision'], label='Val Precision', linestyle='--', alpha=0.7)
            ax.plot(history['val_recall'], label='Val Recall', linestyle='--', alpha=0.7)
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Score')
        ...
        if 'val_loss' in history and 'val_accuracy' in history:
            ...
            p = np.poly1d(z)
            ax.plot(history['val_loss'], p(history['val_loss']), "r--", alpha=0.8, label=f'Trend')
            ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def create_experiment_report(
        ...
        if history_path.exists():
            ...
            metrics_to_show = [
                ('Final Train Loss', history.get('train_loss', [])[-1] if history.get('train_loss') else 'N/A'),
                ('Final Val Loss', history.get('val_loss', [])[-1] if history.get('val_loss') else 'N/A'),
                ('Final Val Accuracy', history.get('val_accuracy', [])[-1] if history.get('val_accuracy') else 'N/A'),
                ('Final Val F1', history.get('val_f1', [])[-1] if history.get('val_f1') else 'N/A'),
                ('Final Val AUC', history.get('val_auc', [])[-1] if history.get('val_auc') else 'N/A'),
                ('Best Val Loss', min(history.get('val_loss', [float('inf')])) if history.get('val_loss') else 'N/A'),
                ('Best Val Accuracy', max(history.get('val_accuracy', [0])) if history.get('val_accuracy') else 'N/A'),
            ]
        ...
        if mlflow_run_id:
            ...
            report_lines.append(f"- **MLflow UI**: [View in MLflow](http://localhost:5000/#/experiments/0/runs/{mlflow_run_id})\n\n")
    ...
    def compare_experiments(
        ...
        for exp_dir in experiment_dirs:
            ...
            if config_path.exists():
                with open(config_path, 'r') as f:
                    config = json.load(f)
                exp_data['learning_rate'] = config.get('training', {}).get('learning_rate', 'N/A')
                exp_data['batch_size'] = config.get('training', {}).get('batch_size', 'N/A')
            ...
            if history_path.exists():
                ...
                for metric in metrics_to_compare:
                    if metric in history and history[metric]:
                        exp_data[f'best_{metric}'] = max(history[metric]) if 'accuracy' in metric else min(history[metric])
        ...
        fig, axes = plt.subplots(1, len(metrics_to_compare), figsize=(6*len(metrics_to_compare), 6))
    ...
    def visualize_predictions(
        ...
        ax.bar(['Not Survived', 'Survived'], [pred_counts.get(0, 0), pred_counts.get(1, 0)])
        ...
        if y_proba is not None:
            ax = axes[1, 0]
            ax.hist(y_proba[y_true == 0], bins=20, alpha=0.5, label='Not Survived', color='red')
            ax.hist(y_proba[y_true == 1], bins=20, alpha=0.5, label='Survived', color='green')
...
def create_mlflow_dashboard_link(tracking_uri: str = "./mlruns") -> str:
    ...
    try:
        ...
        console.print("[blue]Open your browser and navigate to: http://localhost:5000[/blue]")
