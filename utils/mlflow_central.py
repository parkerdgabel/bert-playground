...
class MLflowCentral:
    ...
    def initialize(
        ...
        self._tracking_uri = (
            tracking_uri or os.getenv("MLFLOW_TRACKING_URI") or self.TRACKING_URI
        )
        self._artifact_root = (
            artifact_root or os.getenv("MLFLOW_ARTIFACT_ROOT") or self.ARTIFACT_ROOT
        )
        ...
        try:
            ...
            if "deleted experiment" in str(e):
                logger.warning(
                    f"Experiment '{experiment}' is deleted. Attempting to restore it."
                )
                try:
                    ...
                    logger.error(
                        f"Failed to restore experiment '{experiment}': {restore_error}"
                    )
                    raise MLflowConfigurationError(
    ...
    def validate_connection(self) -> dict[str, Any]:
        ...
        if not self._initialized:
            return {"status": "NOT_INITIALIZED", "message": "MLflow not initialized"}
    ...
    def migrate_experiment(
        ...
        try:
            ...
            if source_experiment is None:
                logger.warning(
                    f"Experiment '{experiment_name}' not found in {source_uri}"
                )
                return
