        ...
        if component_grads:
            ...
            if all_norms and all_values:
                ...
                component_stats[component_name] = {
                    ...
                    'total_norm': float(mx.sqrt(mx.sum(all_values_concat * all_values_concat)))
                }
...
def log_gradient_statistics(
    ...
    logger.info(f"  Total gradient norm: {total_norm:.4f} {'(CLIPPED)' if clipped else ''}")
    
    for component, stats in gradient_stats.items():
        logger.info(f"  {component:12s}: "
...
def detect_gradient_anomalies(
    ...
    for component, stats in gradient_stats.items():
        ...
        if stats['max_norm'] > anomaly_threshold:
            anomalies.append(f"{component} component has exploding gradients: "
                           f"max_norm={stats['max_norm']:.4f}")
        
        # Check for component-specific vanishing gradients
        if stats['mean_norm'] < 1e-7:
            anomalies.append(f"{component} component has vanishing gradients: "
                           f"mean_norm={stats['mean_norm']:.4f}")
        
        # Check for abnormal gradient distribution
        if stats['std_norm'] > stats['mean_norm'] * 5:
            anomalies.append(f"{component} component has highly variable gradients: "
...
class GradientMonitor:
    ...
    def process_gradients(
        ...
        component_stats = monitor_gradient_components(
            gradients, self.component_mapping
        )
        ...
        if should_log:
            log_gradient_statistics(
                component_stats, step, total_norm, was_clipped
            )
...
def create_gradient_monitor(
    ...
    component_mappings = {
        'cnn_hybrid': {
            'embeddings': ['embeddings', 'word_embeddings', 'position_embeddings'],
            'bert': ['bert', 'transformer_layers', 'attention', 'feed_forward', 'self_attn'],
            'cnn': ['conv', 'cnn', 'dilated_conv', 'kernel'],
            'fusion': ['fusion', 'attention_fusion', 'feature_projection'],
            'classifier': ['classifier', 'dense', 'output']
        },
        'bert': {
            'embeddings': ['embeddings', 'word_embeddings', 'position_embeddings'],
            'transformer': ['transformer_layers', 'attention', 'feed_forward', 'self_attn'],
            'classifier': ['classifier', 'dense', 'output']
        }
    }
