...
def train(
    ...
    val_path: Optional[Path] = typer.Option(None, "--val", "-v", help="Path to validation CSV"),
    ...
    model_type: str = typer.Option("base", "--model-type", help="Model type: base or cnn_hybrid"),
    ...
    num_workers: int = typer.Option(4, "--workers", "-w", help="Number of data workers"),
    experiment_name: str = typer.Option("mlx_modernbert", "--experiment", help="MLflow experiment name"),
    ...
    config: Optional[Path] = typer.Option(None, "--config", "-c", help="Load config from JSON file"),
    # CNN-specific options
    cnn_kernel_sizes: str = typer.Option("2,3,4,5", "--cnn-kernels", help="CNN kernel sizes (comma-separated)"),
    ...
    with console.status("[yellow]Creating model...[/yellow]"):
        ...
        if model_type == "cnn_hybrid":
            ...
            # Create base model
            bert_model = create_model(model_name, num_labels=2)
    ...
    try:
        ...
        if trainer.training_history["val_accuracy"]:
            ...
            console.print(f"[green]Best validation accuracy: {best_val_acc:.4f}[/green]")
...
def predict(
    ...
    checkpoint: Path = typer.Option(..., "--checkpoint", "-c", help="Path to model checkpoint"),
    output_path: Path = typer.Option("./submission.csv", "--output", "-o", help="Output submission path"),
    batch_size: int = typer.Option(32, "--batch-size", "-b", help="Batch size"),
    max_length: int = typer.Option(256, "--max-length", help="Maximum sequence length"),
):
    ...
    with console.status("[yellow]Loading model...[/yellow]"):
        from models.classification_head import TitanicClassifier
        bert_model = create_model("answerdotai/ModernBERT-base", num_labels=2)
        model = TitanicClassifier(bert_model)
...
def benchmark(
    ...
    num_steps: int = typer.Option(10, "--steps", "-n", help="Number of steps to benchmark"),
):
    ...
    # Create model
    model = create_optimized_model(model_name, num_labels=2)
...
def export(
    checkpoint: Path = typer.Option(..., "--checkpoint", "-c", help="Path to model checkpoint"),
    output_path: Path = typer.Option(..., "--output", "-o", help="Output path for exported model"),
    format: str = typer.Option("mlx", "--format", "-f", help="Export format (mlx, onnx)"),
):
