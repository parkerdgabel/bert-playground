...
class UnifiedTitanicDataPipeline:
    ...
    def __init__(
        ...
        self._configure_optimization(
            optimization_level, use_mlx_data, pre_tokenize,
            prefetch_size, num_threads, cache_size
        )
    ...
    def _configure_optimization(
        self, level: OptimizationLevel, use_mlx_data: Optional[bool],
        pre_tokenize: Optional[bool], prefetch_size: int,
        num_threads: int, cache_size: int
    ):
    ...
    def _process_batch(self, indices: List[int]) -> Dict[str, mx.array]:
        ...
        labels = mx.array([self.labels[i] for i in indices])
